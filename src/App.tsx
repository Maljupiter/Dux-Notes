...
type ToolIconName = 'select' | 'lasso' | 'pen' | 'highlighter' | 'eraser' | 'text' | 'image' | 'shape' | 'space' | 'hand';
...
const LIGHT_CHROME: Record<string, ThemeChrome> = {
  ecru: {
    ...
    uiText: '#2C1C10',
    uiMuted: '#735F48',
    uiBorder: 'rgba(64, 43, 22, 0.18)',
    uiShadow: 'rgba(64, 43, 22, 0.16)',
    sidebarBg: '#EFE2C8',
    topbarBg: '#FFF3D7',
    toolbarBg: '#F5E8CB',
    buttonBg: '#4D3218',
...
const LABEL_OPTIONS: LabelOption[] = [
  { id: '', name: 'No label', icon: '○', bg: 'var(--chip-bg)', fg: 'var(--chip-fg)' },
...
function updateQuickNoteTextInDocument(doc: DocumentRecord, text: string): DocumentRecord {
  ...
  setPageAnnotations(next, page.key, { strokes: [], imageBoxes: [], shapeBoxes: [], textBoxes: [textBox] });
  next.updatedAt = now;
...
function buildFlashcardKnowledgeBank(): FlashcardBankEntry[] {
  ...
  for (const bank of [...FLASHCARD_TOPIC_BANKS, ...EXPANDED_FLASHCARD_TOPIC_BANKS]) {
...
function generateLocalFlashcardSet(prompt: string, deck: string): { cards: GeneratedFlashcardPreview[]; warning: string } {
  ...
  if (matches.length) {
    return {
      cards: matches.map((entry) => ({ id: makeId(), deck, front: entry.front, back: entry.back })),
      warning: matches.length < count ? `${matches.length} questions found for this topic.` : ''
    };
  }
  return {
...
function getDuxAiGuideMatches(prompt: string, limit = 3) {
  return [...DUX_AI_GUIDES, ...EXPANDED_DUX_AI_GUIDES]
...
function extractAvailabilityWindows(prompt: string): StudyAvailabilityWindow[] {
  ...
  const timePattern = '(\d{1,2}(?::\d{2})?\s*(?:am|pm)?)\s*(?:to|-|until|–|—)\s*(\d{1,2}(?::\d{2})?\s*(?:am|pm)?)';
  const dayToken = '(?:weekdays?|school days?|weekends?|daily|every day|everyday|mondays?|mons?|tuesdays?|tues?|wednesdays?|weds?|thursdays?|thurs?|fridays?|fris?|saturdays?|sats?|sundays?|suns?)';
  const dayPattern = `(${dayToken}(?:\s*(?:,|/|&|\+|and)\s*${dayToken})*)`;
  const dayFirst = new RegExp(`${dayPattern}[^0-9]{0,36}${timePattern}`, 'gi');
  const timeFirstWithDay = new RegExp(`${timePattern}\s*(?:on|for|during)?\s*${dayPattern}`, 'gi');
  const timeOnly = new RegExp(timePattern, 'gi');

  const addWindow = (label: string | undefined, startRaw: string, endRaw: string) => {
    ...
    for (const dayType of dayTypes) {
      const candidate = { dayType, start: startText, end: endText } as StudyAvailabilityWindow;
      const key = `${candidate.dayType}-${candidate.start}-${candidate.end}`;
      if (windows.some((item) => `${item.dayType}-${item.start}-${item.end}` === key)) continue;
  ...
  for (const match of prompt.matchAll(dayFirst)) addWindow(match[1], match[2], match[3]);
  for (const match of prompt.matchAll(timeFirstWithDay)) addWindow(match[3], match[1], match[2]);

  if (!windows.length) {
    for (const match of prompt.matchAll(timeOnly)) addWindow(undefined, match[1], match[2]);
  }
...
function normaliseAnnotationZ(annotations: PageAnnotations): PageAnnotations {
  ...
  return {
    imageBoxes: (annotations.imageBoxes || []).map((item) => ({ ...item, z: nextNumber(item.z) })),
    ...
    textBoxes: (annotations.textBoxes || []).map((item) => ({ ...item, z: nextNumber(item.z) }))
  };
}
...
function getPageAnnotations(doc: DocumentRecord, pageKey: string): PageAnnotations {
  ...
  if (!raw) return { strokes: [], textBoxes: [], imageBoxes: [], shapeBoxes: [] };
  if (Array.isArray(raw)) return normaliseAnnotationZ({ strokes: raw, textBoxes: [], imageBoxes: [], shapeBoxes: [] });
...
function smoothStrokePoints(points: Point[]): Point[] {
  ...
  const cleaned = points.filter((point, index) => index === 0 || Math.hypot(point.x - points[index - 1].x, point.y - points[index - 1].y) > 0.4);
  ...
  for (let i = 0; i < cleaned.length - 1; i += 1) {
    ...
    result.push({
      x: a.x * 0.72 + b.x * 0.28,
      y: a.y * 0.72 + b.y * 0.28,
      pressure: ((a.pressure || 0.5) * 0.72) + ((b.pressure || 0.5) * 0.28)
    });
    result.push({
      x: a.x * 0.28 + b.x * 0.72,
      y: a.y * 0.28 + b.y * 0.72,
      pressure: ((a.pressure || 0.5) * 0.28) + ((b.pressure || 0.5) * 0.72)
    });
  }
...
type ElementKind = 'stroke' | 'text' | 'image' | 'shape';
...
function parseElementKey(key: string): SelectedElement | null {
  ...
  if (!id) return null;
  if (kind !== 'stroke' && kind !== 'text' && kind !== 'image' && kind !== 'shape') return null;
  return { kind, id };
}
...
function moveElements(annotations: PageAnnotations, keys: string[], dx: number, dy: number, bounds: { width: number; totalHeight: number }): PageAnnotations {
  ...
  return {
    ...
    textBoxes: annotations.textBoxes.map((box) => selected.has(elementKey('text', box.id))
      ...
      : box),
    imageBoxes: annotations.imageBoxes.map((box) => selected.has(elementKey('image', box.id))
...
function drawShape(context: CanvasRenderingContext2D, shape: ShapeBox) {
  ...
  if (kind === 'cylinder') {
    context.beginPath(); context.ellipse(rect.x + w / 2, rect.y + h * 0.12, w / 2, h * 0.12, 0, 0, Math.PI * 2); context.stroke();
    context.beginPath(); context.moveTo(rect.x, rect.y + h * 0.12); context.lineTo(rect.x, rect.y + h * 0.88); context.ellipse(rect.x + w / 2, rect.y + h * 0.88, w / 2, h * 0.12, 0, 0, Math.PI); context.lineTo(rect.x + w, rect.y + h * 0.12); context.stroke(); context.restore(); return;
  }
  ...
  for (const entry of items) {
    if (entry.type === 'image') {
      try {
        ...
        context.drawImage(image, entry.item.x, entry.item.y, entry.item.width, entry.item.height);
      } catch (error) {
...
function selectElementsInsideLasso(annotations: PageAnnotations, polygon: Point[]) {
  if (polygon.length < 3) return [];
  const keys: string[] = [];
  const addIfIntersecting = (kind: ElementKind, id: string, bounds: { x: number; y: number; width: number; height: number }) => {
    if (rectIntersectsPolygon(bounds, polygon)) keys.push(elementKey(kind, id));
  };

  for (const stroke of annotations.strokes) {
    if (strokeIntersectsLasso(stroke, polygon)) keys.push(elementKey('stroke', stroke.id));
  }
...
async function paintPageBackground(
  ...
  if (page.kind === 'pdf' && sourceCanvas) {
    let sourceStart = 0;
    let visualStart = 0;
    for (const spacer of sortedSpacers(page.spacers || [])) {
...
async function renderSingleElementOverlay(entry: RenderableElement, size: { width: number; totalHeight: number }, scale = 2) {
  ...
  const annotations: PageAnnotations = { strokes: [], textBoxes: [], imageBoxes: [], shapeBoxes: [] };
...
export default function App() {
  ...
  const currentAnnotations = useMemo(() => {
    if (!activeDoc || !currentPage) return { strokes: [], textBoxes: [], imageBoxes: [], shapeBoxes: [] };
  ...
  const visibleDocuments = useMemo(() => {
    ...
    return source.filter((doc) => {
      ...
      const searchText = [doc.name, folder, getLabelOption(doc.label).name, quickPreview, ...(doc.tags || [])].join(' ').toLowerCase();
  ...
  const hasActiveSecondarySettings = hasSecondaryToolSettings(tool);
  const secondarySettingsVisible = hasActiveSecondarySettings && secondaryBarOpen;
  ...
  function setActivePageIndex(index: number, scroll = false) {
    ...
    if (scroll) {
      window.requestAnimationFrame(() => {
        pageShadowRefs.current[page.key]?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      });
    }
  }
  ...
  const redrawPageOverlay = useCallback(async (pageKey: string, annotations: PageAnnotations, draftStroke?: Stroke | null, draftShape?: ShapeBox | null, lassoPoints?: Point[]) => {
    ...
    await drawRetainedElements(offscreenContext, annotations, zoom * ratio, { draftStroke, draftShape, includeText: false });
    if (lassoPoints?.length) drawLassoPath(offscreenContext, lassoPoints, zoom * ratio);
  ...
  const renderAllPages = useCallback(async () => {
    ...
    for (const [index, page] of activeDoc.pages.entries()) {
      ...
      try {
        const size = await paintPageBackground(base, page, pdfRef.current, zoom * ratio, activeTheme);
  ...
  }, [activeDoc, zoom, activeTheme, redrawPageOverlay]);
    ...
    try {
      ...
      if (nextDoc.pdfFileName) {
        ...
        if (!nextDoc.pages || nextDoc.pages.length === 0) {
          ...
          nextDoc.annotations = {};
          for (const page of nextDoc.pages) setPageAnnotations(nextDoc, page.key, { strokes: [], textBoxes: [], imageBoxes: [], shapeBoxes: [] });
  ...
  async function handleCreateNotebook() {
    ...
    try {
      ...
      const doc: DocumentRecord = {
        ...
        updatedAt: now,
        pages,
        annotations: Object.fromEntries(pages.map((page) => [page.key, { strokes: [], textBoxes: [], imageBoxes: [], shapeBoxes: [] }]))
      };
  ...
  async function generateScheduleWithAi() {
    ...
    try {
      ...
      const now = new Date().toISOString();
      const newItems: StudyScheduleItem[] = [];
      ...
      for (const date = new Date(startDate); date.getTime() <= finalDate.getTime(); date.setDate(date.getDate() + 1)) {
        ...
        for (const windowItem of dayWindows) {
          ...
          while (cursor + duration <= windowEnd && newItems.length < 80) {
            ...
            newItems.push({
              id: makeId(),
              title: makeGeneratedSessionTitle(subject, subjectIndex),
              date: iso,
              startTime: minutesToTime(cursor),
              endTime: minutesToTime(cursor + duration),
              subject,
              note: `Generated locally from your plan. Session length: ${duration} minutes. Break after: ${breakMinutes} minutes.`,
      ...
      if (!newItems.length) {
        setAiError('No sessions could be created. Try adding available times like “4pm to 6pm weekdays”.');
        return;
      }
      ...
      setStatus(`Added ${newItems.length} sessions across ${uniqueDays} days with the local generator.`);
    } catch (error) {
      console.error(error);
      setAiError('The free generator could not create a schedule. Try a simpler request.');
    } finally {
      setAiLoading(false);
    }
  }
  ...
  function runLocalFlashcardGenerator() {
    ...
    setFlashcardGeneratorWarning(generated.warning);
  ...
  function handleSetLabel(label: LabelId | '') {
    ...
    setStatus(label ? `Label set to ${option.name}.` : 'Label cleared.');
  }
  ...
  function handleInsertBlankPage() {
    if (!activeDocRef.current) return;
    const data = getActivePageData();
    const template = data?.page.kind === 'blank' ? data.page.template : newNotebookTemplate;
    const blankPage = createBlankPage(template || 'ruled');
    const insertAfter = data ? data.index + 1 : 0;
    applyDocumentUpdate((doc) => {
      doc.pages.splice(insertAfter, 0, blankPage);
      setPageAnnotations(doc, blankPage.key, { strokes: [], textBoxes: [], imageBoxes: [], shapeBoxes: [] });
      return doc;
    });
    ...
    setStatus('Inserted a blank page.');
  }
  ...
  function handleClearPage() {
    ...
    if (!confirmed) return;
    applyDocumentUpdate((doc) => {
      setPageAnnotations(doc, data.page.key, { strokes: [], textBoxes: [], imageBoxes: [], shapeBoxes: [] });
      return doc;
    });
  ...
  function handleDeleteCurrentPage() {
    const data = getActivePageData();
    if (!data || !activeDocRef.current || isQuickNoteDoc(activeDocRef.current)) return;
    if (data.doc.pages.length <= 1) {
      window.alert('This notebook only has one page. Use Clear page if you want to empty it.');
      return;
    }
    const confirmed = window.confirm(`Delete page ${data.index + 1}? This removes the page and everything written on it.`);
    if (!confirmed) return;
    const nextIndex = clamp(data.index, 0, data.doc.pages.length - 2);
    applyDocumentUpdate((doc) => {
      const removed = doc.pages.splice(data.index, 1)[0];
      if (removed?.key && doc.annotations) delete doc.annotations[removed.key];
      return doc;
    });
    pageIndexRef.current = nextIndex;
    setPageIndex(nextIndex);
    ...
    clearSelection();
    setStatus(`Deleted page ${data.index + 1}.`);
  }



  function splitStrokeAroundPoint(stroke: Stroke, point: Point, radius: number): Stroke[] {
    const segments: Point[][] = [];
    let current: Point[] = [];
    ...
    return segments.map((points, index) => ({
      ...stroke,
      id: `${stroke.id}-pixel-${makeId()}-${index}`,
      points
    }));
  }
  ...
  function eraseAt(point: Point) {
    ...
    applyDocumentUpdate((doc) => {
      ...
      if (eraserMode === 'pixel') {
        const strokes = annotations.strokes.flatMap((stroke) => splitStrokeAroundPoint(stroke, point, radius));
        setPageAnnotations(doc, data.page.key, { ...annotations, strokes });
        return doc;
      }

      const strokes = annotations.strokes.filter((stroke) => !strokeIsNearPoint(stroke, point, radius));
      const textBoxes = annotations.textBoxes.filter((box) => {
        const inside = point.x >= box.x && point.x <= box.x + box.width && point.y >= box.y && point.y <= box.y + box.minHeight;
        return !inside;
      });
  ...
  function scheduleStagePinchZoom(center: { clientX: number; clientY: number }, distance: number) {
    ...
    const nextZoom = clamp(pan.startZoom * (distance / pan.startDistance), 0.65, 2.1);
  ...
  function handleCanvasStageTouchMove(event: ReactTouchEvent<HTMLElement>) {
    if (event.touches.length >= 2 && !hasActiveInkGesture()) {
      ...
      if (pan && center) {
        ...
        if (distanceDelta > 8 || pan.mode === 'pinch') {
  ...
  function handlePointerDown(event: ReactPointerEvent<HTMLCanvasElement>, indexOverride?: number) {
    ...
    void redrawPageOverlay(page.key, annotations, stroke);
  }
  ...
  function handlePointerMove(event: ReactPointerEvent<HTMLCanvasElement>, indexOverride?: number) {
    ...
    if (tool === 'lasso' && event.buttons === 1 && lassoPointsRef.current.length) {
    ...
    if (currentShapeRef.current && event.buttons === 1) {
    ...
    if (!currentStrokeRef.current || event.buttons !== 1) return;
    for (const nextPoint of points) {
      if (shouldKeepStrokePoint(currentStrokeRef.current.points, nextPoint, 0.55)) currentStrokeRef.current.points.push(nextPoint);
    }
    scheduleDraftOverlayRedraw(activePage.key, annotations, currentStrokeRef.current);
  }
  ...
  function handlePointerUp(event?: ReactPointerEvent<HTMLCanvasElement>) {
    ...
    if (lassoPointsRef.current.length && page) {
      let selected = selectElementsInsideLasso(annotations, lassoPointsRef.current);
    ...
    if (currentShapeRef.current && page) {
      ...
      setStatus('Shape added. Use Select to move or delete it.');
      return;
    }
    ...
    const savedStroke = { ...finishedStroke, points: smoothStrokePoints(finishedStroke.points) };
    applyDocumentUpdate((draft) => {
      const nextAnnotations = getPageAnnotations(draft, page.key);
  ...
  async function buildEditableSidecar() {
    ...
    return {
      ...
      note: 'Keep this .localnotes.json file next to the exported PDF if you want to re-import editable strokes, text, images, shapes, spacers, labels, and themes.',
      document: cleanDoc,
      originalPdfBase64
    };
  }
  ...
  async function handleExportPdf() {
    if (!activeDoc) return;
    const docAtExportStart = normaliseDocument(cloneDocument(activeDocRef.current || activeDoc), activeThemeId);
    setIsExporting(true);
    ...
    try {
      ...
      bytes = await exportDoc.save();
      sidecar = await buildEditableSidecar();
      ...
      try {
        if (mappedPath && mappedPath !== 'Browser folder access granted' && window.localNotes.savePdfExportToFolder) {
          filePath = await window.localNotes.savePdfExportToFolder(mappedPath, fileName, pdfBuffer as ArrayBuffer as unknown as Uint8Array, sidecar);
        ...
        if (!filePath) {
          filePath = await window.localNotes.savePdfExport(fileName, pdfBuffer as ArrayBuffer as unknown as Uint8Array, sidecar);
        }
  ...
  function renderSelectionOverlaysForPage(page: NotebookPage, annotations: PageAnnotations) {
    ...
    return (
      <div
        ...
        className="selection-box unified"
        style={style}
        onPointerDown={(event) => startSelectionDrag(event, primary.kind, primary.id, page.key)}
  ...
  function renderSpacerControlsForPage(page: NotebookPage, index: number, view: { width: number; baseHeight: number; totalHeight: number }) {
    ...
    return sortedSpacers(page.spacers || []).map((spacer) => {
      ...
      return (
        ...
        >
          <span>{Math.round(spacer.height)}px space</span>
          <button
            type="button"
            className="spacer-resize-handle"
            title="Drag to resize this inserted space"
            onPointerDown={(event) => handleSpacerResizeStart(event, spacer, page.key, index)}
            onPointerMove={handleSpacerResizeMove}
            onPointerUp={handleSpacerResizeEnd}
            onPointerCancel={handleSpacerResizeEnd}
          />
        </div>
      );
    });
          ...
          <div className={`image-layer ${tool === 'select' ? 'interactive' : ''}`}>
            {annotations.imageBoxes.map((box) => {
              ...
              return (
                <div
                  key={box.id}
                  className={`image-box-frame ${selected ? 'selected' : ''}`}
                  style={style}
                ...
                >
                  {selected && (
                    <>
                      <button className="image-delete-button" onClick={(event) => { event.preventDefault(); event.stopPropagation(); handleDeleteImage(box.id, true, page.key); }}>×</button>
                      <button className="image-resize-handle corner" onPointerDown={(event) => handleImageResizeStart(event, box, 'corner', page.key)} onPointerMove={handleImageResizeMove} onPointerUp={handleImageResizeEnd} onPointerCancel={handleImageResizeEnd} />
                      <button className="image-resize-handle edge right" onPointerDown={(event) => handleImageResizeStart(event, box, 'edge-right', page.key)} onPointerMove={handleImageResizeMove} onPointerUp={handleImageResizeEnd} onPointerCancel={handleImageResizeEnd} />
                      <button className="image-resize-handle edge bottom" onPointerDown={(event) => handleImageResizeStart(event, box, 'edge-bottom', page.key)} onPointerMove={handleImageResizeMove} onPointerUp={handleImageResizeEnd} onPointerCancel={handleImageResizeEnd} />
                      <button className="image-resize-handle edge left" onPointerDown={(event) => handleImageResizeStart(event, box, 'edge-left', page.key)} onPointerMove={handleImageResizeMove} onPointerUp={handleImageResizeEnd} onPointerCancel={handleImageResizeEnd} />
                      <button className="image-resize-handle edge top" onPointerDown={(event) => handleImageResizeStart(event, box, 'edge-top', page.key)} onPointerMove={handleImageResizeMove} onPointerUp={handleImageResizeEnd} onPointerCancel={handleImageResizeEnd} />
                    </>
                  )}
                </div>
              );
...
Answer: **x = ${formatNumber(r1)}** or **x = ${formatNumber(r2)}**.`;
  ...
  async function sendAiChatMessage() {
    ...
    const context = {
      ...
      label: activeDoc ? getLabelOption(activeDoc.label).name : '',
...
Offline note: I could not reach the Gemini backend, so I used the built-in Dux AI helper for this reply.`;
  ...
  return (
    <div className={`app-shell ${uiMode === 'dark' ? 'dark-ui' : ''} ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`} style={appStyle}>
      <aside className={`sidebar ${sidebarCollapsed ? 'collapsed' : ''}`}>
        ...
        <div className="search-block">
          <input
            ...
            placeholder="Search notebooks, notes, labels, folders"
          />
        </div>
            ...
            return (
              <article key={doc.id} className={`document-card ${activeDoc?.id === doc.id ? 'active' : ''}`}>
                <button className="document-main" onClick={() => sidebarMode === 'trash' ? undefined : loadDocument(doc)}>
                  <div className="doc-thumbnail" style={{ background: docTheme.background, color: docTheme.text, borderColor: docTheme.accent }}>
                  ...
                  <div className="doc-info">
                    ...
                    <div className="doc-meta-row">
                      <small>{folder}</small>
                      {doc.label && (() => {
                        const label = getLabelOption(doc.label);
        ...
        <div className="sidebar-study-footer">
          <button className="study-schedule-button sidebar-study-button" onClick={openStudyPlanner}>📅 Study schedule</button>
          <button className="study-schedule-button sidebar-flashcard-button" onClick={() => { setFlashcardModalOpen(true); resetFlashcardReview(); }}>🃏 Flashcards</button>
        </div>
      </aside>
      ...
      <main className="workspace">
        <header className="topbar">
          ...
          {activeDoc && (() => {
            const label = getLabelOption(activeDoc.label);
            return (
        ...
        {!isQuickNoteDoc(activeDoc) && <section className="toolbar" aria-label="Notebook tools">
          <div className="tool-clusters">
            ...
            <div className="tool-group main-tools icon-toolbar" aria-label="Text and image tools">
              ...
              <button title="Add image" data-tooltip="Image" aria-label="Add image" onClick={() => imageInputRef.current?.click()} disabled={!currentPage}><ToolIcon name="image" /></button>
          ...
          <div className="more-menu-wrap">
            ...
            {moreMenuOpen && (
              <div className="more-menu">
                ...
                <button onClick={() => { handleUndo(); setMoreMenuOpen(false); }} disabled={undoCount === 0}>Undo</button>
                <button className="danger-button" onClick={() => { handleClearPage(); setMoreMenuOpen(false); }} disabled={!currentPage}>Clear page</button>
              </div>
            )}
          </div>

          <div className="toolbar-quick-actions" aria-label="Scratch and Dux AI">
            <button className="quick-note-toggle" data-tooltip="Scratch" onClick={(event) => { event.stopPropagation(); setScratchpadOpen((open) => !open); }} title="Scratch, Cmd/Ctrl+Shift+Space" aria-label="Scratch">✏</button>
            <button className="ask-ai-toggle" data-tooltip="Dux AI" onClick={() => { setTipPopupOpen(false); setAiChatOpen((open) => !open); }} title="Ask Dux AI" aria-label="Ask Dux AI">🤖</button>
          </div>

          <div className={`context-panel ${secondarySettingsVisible ? 'is-open' : 'is-collapsed'} ${isInking && (tool === 'pen' || tool === 'highlighter') ? 'is-inking-fade' : ''}`} aria-hidden={!secondarySettingsVisible}>
            {tool === 'pen' && (
              <>
            ...
            {tool === 'eraser' && (
              <>
                ...
                <span className="context-hint">Pixel removes parts of strokes. Object removes whole strokes, text, images or shapes.</span>
              </>
            )}
            {tool === 'text' && (
              <>
                ...
                <label>
                  Size
                  <input type="number" min="8" max="80" value={fontSize} onChange={(event) => setFontSize(Number(event.target.value))} />
                </label>
                <label>
                  Weight
                  <select value={fontWeight} onChange={(event) => setFontWeight(event.target.value as '400' | '600' | '700')}>
            ...
            {tool === 'shape' && (
              <>
                <label>
                  Shape
                  <select value={shapeKind} onChange={(event) => setShapeKind(event.target.value as ShapeKind)}>
                ...
                {renderColourPicker('shapeStroke', 'Shape border', shapeStrokeColour, SHAPE_PRESETS.filter((item) => item.value !== 'transparent'), setShapeStrokeColour)}
                {renderColourPicker('shapeFill', 'Shape fill', shapeFillColour, SHAPE_PRESETS, setShapeFillColour)}
                <label>
                  Width
                  <input type="range" min="1" max="18" value={strokeWidth} onChange={(event) => setStrokeWidth(Number(event.target.value))} />
                  <b>{strokeWidth}px</b>
            ...
            {tool === 'lasso' && <span className="context-hint">Draw around items to select them together. Press Delete to remove the selection.</span>}
            {tool === 'space' && (
              <label>
                Inserted space
          ...
          <div className="toolbar-actions">
            <button data-tooltip="Undo" title="Undo" aria-label="Undo" onClick={handleUndo} disabled={undoCount === 0}>↶</button>
            <button className="danger-button" data-tooltip="Clear page" title="Clear page" aria-label="Clear page" onClick={handleClearPage} disabled={!currentPage}>🧹</button>
          </div>
        </section>}
            ...
            <div className="page-rail" aria-label="Page thumbnails">
              {activeDoc.pages.map((page, index) => (
                <button
                  ...
                  onClick={() => setActivePageIndex(index, true)}
                  title={`Go to page ${index + 1}`}
                >
                  <span className="page-thumb-paper">
                    {page.kind === 'blank' ? (page.template === 'grid' ? '▦' : page.template === 'plain' ? '□' : '☰') : 'PDF'}
                  </span>
                  <b>{index + 1}</b>
                </button>
              ))}
        ...
        <section className="bottom-bar" aria-label="Workspace status and controls">
              ...
              {tipPopupOpen && (
                <div ref={tipPopupRef} className="tip-popup" style={tipPopupPosition ? ({ '--tip-left': `${tipPopupPosition.left}px`, '--tip-bottom': `${tipPopupPosition.bottom}px` } as CSSProperties) : undefined} role="dialog" aria-label="Study tip">
                  ...
                  <div className="tip-popup-actions">
                    ...
                    <button type="button" onClick={() => { setTipPopupOpen(false); setStudyPlannerOpen(true); }}>Plan study</button>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="bottom-control-group" aria-label="Page, zoom and AI controls">
            ...
            <label className="zoom-control" title="Zoom canvas">
              Zoom
              <input type="range" min="0.65" max="2.1" step="0.05" value={zoom} onChange={(event) => setZoom(Number(event.target.value))} />
      ...
      {flashcardCreateOpen && !flashcardModalOpen && (
        <div className="modal-backdrop" role="dialog" aria-modal="true">
          <div className="flashcard-create-modal">
      ...
      {flashcardModalOpen && (
        <div className="modal-backdrop" role="dialog" aria-modal="true">
          <div className="flashcard-review-modal">
            <div className="modal-heading">
              <div>
                <h2>Flashcards</h2>
            ...
            {!reviewStarted ? (
              flashcardCreateOpen ? (
                ...
                <section className="flashcard-generator-panel">
                  <div className="generator-note">
                    <strong>✨ Local flashcard generator</strong>
                    <span>This generator uses the built-in question bank when it recognises your topic. If the topic is not covered yet, it creates prompt cards you can finish from your notes.</span>
                  </div>
              ...
              <div className="flashcard-start-grid">
                <section className="deck-list-panel">
                  <div className="study-date-heading">
                    ...
                    <div className="flashcard-heading-actions"><button className="secondary-button" onClick={openManualFlashcard}>Add card</button><button className="secondary-button" onClick={openFlashcardGenerator}>✨ Generate flashcards</button></div>
                  </div>
      ...
      {helpOpen && (
        <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Dux Notes help">
          <div className="create-modal help-modal">
            ...
            <div className="help-grid">
              <section>
                ...
                <ul>
                  ...
                  <li><b>Lasso</b> draws around objects to select a group.</li>
                  <li><b>Pen</b> writes freehand. Pick colour, width and opacity in the settings panel.</li>
                  ...
                  <li><b>Text</b> inserts typed boxes. Use the settings panel for font, size and weight.</li>
                  <li><b>Image</b> inserts PNG, JPG, WebP or GIF images onto the page.</li>
                  <li><b>Shape</b> adds diagrams, arrows, frames and flowchart-style symbols.</li>
              ...
              <section>
                <h3>Page controls</h3>
                <ul>
                  <li><b>Blank page</b> adds a clean page after the current page.</li>
                  <li><b>Delete page</b> removes the current page. It is disabled if there is only one page.</li>
                  <li><b>Page box</b> lets you type a page number and press Enter to jump there.</li>
                  <li><b>Zoom</b> changes the page size. On iPad, pinch with two fingers to zoom.</li>
                  <li><b>More</b> has Space below, PNG page, Undo and Clear page.</li>
              ...
              <section>
                ...
                <ul>
                  ...
                  <li>Finger and palm touches are guarded so they do not create random ink.</li>
                  <li>Tap the same tool a second time to hide its extra settings panel.</li>
                  <li>While writing with Pen or Highlighter, the settings panel fades so it stays out of your way.</li>
                </ul>
              </section>
            ...
            <div className="help-tips">
              <strong>Fast workflow:</strong> Import or create a PDF, pick Pen, write, tap Pen again to hide settings, use two fingers to move around, then Save PDF when finished.
            </div>
          </div>
        </div>
      )}
      ...
      {studyPlannerOpen && (
        <div className="modal-backdrop" role="dialog" aria-modal="true">
          <div className={`create-modal study-modal ${aiPanelOpen ? 'ai-mode' : ''}`}>
            <div className="modal-heading">
              <div>
            ...
            {aiPanelOpen && (
              <section className="ai-generator-panel">
                <div className="ai-generator-heading">
                  <div>
                    <strong>Free local study schedule generator</strong>
                    <p>Describe your exams, available times, session length and breaks. Dux Notes will create the sessions on this computer with the local planner.</p>
                  </div>
                  <button className="ghost-button" onClick={() => setAiPanelOpen(false)}>Back to calendar</button>
                </div>

                <div className="ai-key-card">
                  <strong>Runs on this computer</strong>
                  <p>The planner reads your subjects, available time windows, session length and breaks, then fills the calendar with study blocks.</p>
                </div>
                ...
                <div className="modal-actions">
                  ...
                  <button className="primary-button" onClick={generateScheduleWithAi} disabled={aiLoading}>
                    ...
                    {aiLoading ? 'Generating...' : 'Generate free schedule'}
                  </button>
                </div>
              </section>
            )}
