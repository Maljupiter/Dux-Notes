...
import { decompress as decompressZstdFrame } from 'fzstd';
import type {
  BlankPageTemplate,
  DocumentRecord,
  ImageBox,
...
const SCRIBBLE_ERASE_HOLD_MS = 330;
const SCRIBBLE_ERASE_HOLD_MOVE_TOLERANCE = 2.4;
...
type AiChatFrameDragMode = 'move' | 'resize-nw' | 'resize-ne' | 'resize-sw' | 'resize-se';
...
const ONBOARDING_STEPS: OnboardingStep[] = [
  {
    ...
    {
      title: 'Scratch out mistakes',
      text: 'Draw a quick four-zigzag scribble across a stroke, then hold the pen still for a third of a second before lifting. Dux Notes treats that as an intentional scratch-erase instead of random doodling.',
      target: '[data-tour-id="toolbar"]',
      animationClass: 'tour-scribble-erase'
    },
  {
...
const AI_CHAT_TABS_STORAGE_KEY = 'dux-notes-ai-chat-tabs';
const AI_CHAT_ACTIVE_TAB_STORAGE_KEY = 'dux-notes-ai-chat-active-tab';
const AI_CHAT_FRAME_STORAGE_KEY = 'dux-notes-ai-chat-frame';
const AI_CHAT_ZOOM_STORAGE_KEY = 'dux-notes-ai-chat-zoom';
const AI_CHAT_MEMORY_LIMIT = 24;
const AI_CHAT_TAB_LIMIT = 36;
const AI_CHAT_MIN_WIDTH = 320;
const AI_CHAT_MIN_HEIGHT = 280;
const AI_CHAT_MARGIN = 12;
const AI_CHAT_ZOOM_LEVELS = [0.9, 1, 1.15, 1.3, 1.5];
...
type Flashcard = {
  ...
  front: string;
  back: string;
  frontMedia?: FlashcardMedia[];
  backMedia?: FlashcardMedia[];
  dueDate: string;
  linkedDocId?: string | null;
  linkedDocName?: string | null;
  createdAt: string;
...
type ImportedAnkiCard = {
  front: string;
  back: string;
  frontMediaRefs?: string[];
  backMediaRefs?: string[];
  frontMedia?: FlashcardMedia[];
  backMedia?: FlashcardMedia[];
};
  ...
  try {
    ...
    return parsed
      ...
      .map((item) => ({
        ...
        frontMedia: normaliseFlashcardMedia(item.frontMedia),
        backMedia: normaliseFlashcardMedia(item.backMedia),
...
function normaliseFlashcardMedia(value: unknown): FlashcardMedia[] {
  if (!Array.isArray(value)) return [];
  return value
    .filter((item): item is FlashcardMedia => (
      Boolean(item) &&
      typeof item === 'object' &&
      (item as FlashcardMedia).type === 'image' &&
      typeof (item as FlashcardMedia).dataUrl === 'string' &&
      (item as FlashcardMedia).dataUrl.startsWith('data:image/') &&
      typeof (item as FlashcardMedia).name === 'string'
    ))
    .slice(0, 6);
}

function flashcardSideTextPreview(text: string, media?: FlashcardMedia[]) {
  const clean = text.trim();
  const mediaLabel = media?.length ? `${media.length} image${media.length === 1 ? '' : 's'}` : '';
  if (clean && mediaLabel) return `${clean} (+ ${mediaLabel})`;
...
const MAX_ANKI_MEDIA_BYTES = 5 * 1024 * 1024;
const MAX_ANKI_MEDIA_PER_SIDE = 4;
...
async function decompressZstdBytes(data: Uint8Array) {
  try {
    return await decompressBytesWithStream(data, 'zstd');
  } catch (browserError) {
    try {
      return decompressZstdFrame(data);
    } catch {
      // Keep falling through: Electron has a native zstd bridge, and older
      // browsers still get a clear export/update instruction if both fail.
    }

    if (window.localNotes?.decompressZstd) {
      try {
        const decompressed = await window.localNotes.decompressZstd(bytesToLocalNotesPayload(data));
        return decompressed instanceof Uint8Array ? decompressed : new Uint8Array(decompressed);
      } catch (electronError) {
        throw new Error(electronError instanceof Error ? electronError.message : 'The desktop app could not decode this Anki package.');
      }
    }
...
async function readZipEntryBytes(bytes: Uint8Array, entry: ZipDirectoryEntry) {
...
function normaliseAnkiMediaName(value: string) {
...
function dedupeAnkiMediaRefs(refs: string[]) {
  const seen = new Set<string>();
  const output: string[] = [];
  for (const ref of refs) {
    const clean = normaliseAnkiMediaName(ref);
...
function extractAnkiMediaRefs(value: string) {
  const refs: string[] = [];
  String(value || '').replace(/<img\b[^>]*\bsrc=(?:"([^"]+)"|'([^']+)'|([^\s>]+))[^>]*>/gi, (_match, doubleQuoted, singleQuoted, bare) => {
    const clean = normaliseAnkiMediaName(doubleQuoted || singleQuoted || bare || '');
    if (clean) refs.push(clean);
    return '';
  });
  return dedupeAnkiMediaRefs(refs);
}

function removeAnkiMediaTags(value: string) {
  return String(value || '').replace(/<img\b[^>]*>/gi, ' ');
}

function mimeTypeForAnkiMedia(name: string) {
...
function isSupportedAnkiImage(name: string) {
...
async function prepareAnkiCollectionDatabase(entryName: string, bytes: Uint8Array) {
  ...
  if (entryName.toLowerCase() === 'collection.anki21b') {
    try {
      const decompressed = await decompressZstdBytes(bytes);
      if (isSqliteDatabase(decompressed)) return decompressed;
    } catch {
      throw new Error('This is a newer Anki package that uses zstd-compressed collection.anki21b. Update to the latest Dux Notes desktop app to import it directly, or export from Anki with “Support older Anki versions” enabled / “Notes in Plain Text”.');
    }
  }
...
function stripAnkiMarkup(value: string) {
  const withMediaPlaceholders = removeAnkiMediaTags(value)
...
function makeImportedAnkiCard(
  front: string,
  back: string,
  frontMediaRefs: string[] = [],
  backMediaRefs: string[] = []
): ImportedAnkiCard | null {
  const cleanFront = front.trim();
  const cleanBack = removeRepeatedFrontFromBack(cleanFront, back.trim());
  const cleanFrontRefs = dedupeAnkiMediaRefs(frontMediaRefs);
  const cleanBackRefs = dedupeAnkiMediaRefs(backMediaRefs).filter((ref) => !cleanFrontRefs.includes(ref));
  if ((!cleanFront && !cleanFrontRefs.length) || (!cleanBack && !cleanBackRefs.length)) return null;
  return {
    front: cleanFront,
    back: cleanBack,
    frontMediaRefs: cleanFrontRefs,
    backMediaRefs: cleanBackRefs
  };
}

function fieldsToFallbackCard(fields: Array<{ name: string; value: string }>, modelName = 'custom Anki note'): ImportedAnkiCard | null {
  const cleaned = fields
    .map((field) => ({ name: field.name, value: stripAnkiMarkup(field.value), mediaRefs: extractAnkiMediaRefs(field.value) }))
    .filter((field) => field.value || field.mediaRefs.length);
  if (!cleaned.length) return null;

  const frontField = cleaned.find((field) => field.value && /\b(front|question|prompt|term|text)\b/i.test(field.name)) || cleaned.find((field) => field.value) || cleaned[0];
  ...
  const back = [...backFields, ...otherFields]
    ...
    .join('\n\n')
    .trim();
  const backMediaRefs = [...backFields, ...otherFields].flatMap((field) => field.mediaRefs);

  if (frontField.value && (back || backMediaRefs.length)) return makeImportedAnkiCard(frontField.value, back, frontField.mediaRefs, backMediaRefs);
  if (cleaned.length >= 2) {
    const first = cleaned[0];
    const rest = cleaned.slice(1);
    return makeImportedAnkiCard(
      first.value,
      rest.map((field) => `${field.name}: ${field.value}`).join('\n\n'),
      first.mediaRefs,
      rest.flatMap((field) => field.mediaRefs)
    );
  }
  return makeImportedAnkiCard(
    cleaned[0].value,
    `Imported from Anki note type "${modelName}". This note only exposed one readable field, so Dux Notes kept it as a simple review card.`,
    cleaned[0].mediaRefs
  ) || {
...
function ankiNoteToCard(note: AnkiNoteData, model?: AnkiModel, ord = 0): ImportedAnkiCard | null {
  ...
  if (template?.qfmt || template?.afmt) {
    ...
    const back = stripAnkiMarkup(backHtml);
    const card = makeImportedAnkiCard(front, back, extractAnkiMediaRefs(frontHtml), extractAnkiMediaRefs(backHtml));
    if (card) return card;
  }
  ...
  if (/\{\{c\d+::/i.test(combinedRaw)) {
    ...
    const back = stripAnkiMarkup(backRaw);
    const card = makeImportedAnkiCard(front, back, extractAnkiMediaRefs(frontRaw), extractAnkiMediaRefs(backRaw));
    if (card) return card;
  }
...
function dedupeImportedCards(cards: ImportedAnkiCard[]) {
  ...
  for (const card of cards) {
    ...
    const frontRefs = dedupeAnkiMediaRefs(card.frontMediaRefs || []);
    const backRefs = dedupeAnkiMediaRefs(card.backMediaRefs || []);
    const key = normaliseSearchText(`${front} ${back} ${frontRefs.join(' ')} ${backRefs.join(' ')}`);
    const hasFront = front || frontRefs.length || card.frontMedia?.length;
    const hasBack = back || backRefs.length || card.backMedia?.length;
    if (!hasFront || !hasBack || seen.has(key)) continue;
    seen.add(key);
    output.push({
      ...card,
      front,
      back,
      frontMediaRefs: frontRefs,
      backMediaRefs: backRefs,
      frontMedia: normaliseFlashcardMedia(card.frontMedia),
      backMedia: normaliseFlashcardMedia(card.backMedia)
    });
...
function parseDelimitedFlashcards(text: string) {
  ...
  for (const line of lines) {
    ...
    const card = makeImportedAnkiCard(front, back);
    if (card) cards.push(card);
  }
  return dedupeImportedCards(cards);
}

async function attachAnkiMediaToCards(packageBytes: Uint8Array, cards: ImportedAnkiCard[]) {
  if (!cards.some((card) => card.frontMediaRefs?.length || card.backMediaRefs?.length)) return cards;
  let entries: ZipDirectoryEntry[] = [];
  try {
    entries = parseZipDirectory(packageBytes);
  } catch {
    return cards;
  }

  const mediaMapEntry = entries.find((entry) => entry.name === 'media');
  const refToEntryName = new Map<string, string>();
  if (mediaMapEntry) {
    try {
      const mediaMapRaw = new TextDecoder('utf-8').decode(await readZipEntryBytes(packageBytes, mediaMapEntry));
      const mediaMap = JSON.parse(mediaMapRaw) as Record<string, string>;
      Object.entries(mediaMap || {}).forEach(([entryName, fileName]) => {
        const clean = normaliseAnkiMediaName(fileName);
        if (clean) refToEntryName.set(clean.toLowerCase(), entryName);
      });
    } catch {
      // Older or damaged decks can still fall back to direct filename matching.
    }
  }

  const directEntries = new Map<string, ZipDirectoryEntry>();
  entries.forEach((entry) => {
    directEntries.set(entry.name.toLowerCase(), entry);
    const basename = normaliseAnkiMediaName(entry.name);
    if (basename) directEntries.set(basename.toLowerCase(), entry);
  });

  const mediaCache = new Map<string, Promise<FlashcardMedia | null>>();
  const loadMedia = (ref: string) => {
    const clean = normaliseAnkiMediaName(ref);
    const cacheKey = clean.toLowerCase();
    if (!clean || !isSupportedAnkiImage(clean)) return Promise.resolve(null);
    if (mediaCache.has(cacheKey)) return mediaCache.get(cacheKey) as Promise<FlashcardMedia | null>;
    const promise = (async () => {
      const mappedEntryName = refToEntryName.get(cacheKey);
      const entry = (mappedEntryName ? directEntries.get(mappedEntryName.toLowerCase()) : null)
        || directEntries.get(clean.toLowerCase())
        || entries.find((item) => normaliseAnkiMediaName(item.name).toLowerCase() === cacheKey)
        || null;
      if (!entry || entry.uncompressedSize > MAX_ANKI_MEDIA_BYTES) return null;
      try {
        const bytes = await readZipEntryBytes(packageBytes, entry);
        return {
          type: 'image' as const,
          name: clean,
          dataUrl: `data:${mimeTypeForAnkiMedia(clean)};base64,${uint8ArrayToBase64(bytes)}`
        };
      } catch {
        return null;
      }
  ...
  return Promise.all(cards.map(async (card) => {
    const frontMedia = (await Promise.all(dedupeAnkiMediaRefs(card.frontMediaRefs || []).slice(0, MAX_ANKI_MEDIA_PER_SIDE).map(loadMedia))).filter(Boolean) as FlashcardMedia[];
    const backMedia = (await Promise.all(dedupeAnkiMediaRefs(card.backMediaRefs || []).slice(0, MAX_ANKI_MEDIA_PER_SIDE).map(loadMedia))).filter(Boolean) as FlashcardMedia[];
    return {
      ...card,
      frontMedia,
      backMedia
...
async function parseAnkiImportFile(file: File): Promise<AnkiImportPreview> {
  ...
  if (lowerName.endsWith('.apkg') || lowerName.endsWith('.apkg2') || lowerName.endsWith('.apkj') || isZipLike) {
    ...
    cards = await attachAnkiMediaToCards(bytes, cards);
  } else {
...
function addMinutesToClockTime(time: string, minutes: number) {
  return minutesToTime(timeToMinutes(time) + minutes);
}

function minimumEndTimeForStart(startTime: string) {
  return addMinutesToClockTime(startTime || '00:00', 1);
}

function repairEndTimeAfterStart(startTime: string, endTime: string, defaultMinutes = 60) {
  const start = isValidTime(startTime) ? startTime : '16:00';
  const fallbackEnd = minutesToTime(Math.min(23 * 60 + 59, timeToMinutes(start) + defaultMinutes));
  const end = isValidTime(endTime) ? endTime : fallbackEnd;
  return timeToMinutes(end) <= timeToMinutes(start) ? fallbackEnd : end;
}
...
function normaliseAiQuestionMemory(value: unknown) {
  return typeof value === 'string' ? value.trim().slice(0, 5200) : '';
}

function extractAiQuestionNumbers(value: string) {
  const found = value.matchAll(/\b(?:question|q)\s*#?\s*(\d{1,3}[a-z]?)\b/gi);
  return [...new Set([...found].map((match) => match[1].toLowerCase()))].slice(0, 8);
}

function normaliseAiQuestionNumbers(value: unknown) {
  if (!Array.isArray(value)) return [];
  return [...new Set(value.filter((item): item is string => typeof item === 'string').map((item) => item.trim().toLowerCase()).filter((item) => /^\d{1,3}[a-z]?$/.test(item)))].slice(0, 8);
}
  ...
  try {
    ...
    return parsed
      ...
      .map((message) => ({
        ...
        questionMemory: normaliseAiQuestionMemory(message.questionMemory) || undefined,
        questionNumbers: normaliseAiQuestionNumbers(message.questionNumbers)
      }))
...
function normaliseAiChatMessages(input: unknown): ChatMessage[] {
...
function createAiChatTabForDocument(doc?: Pick<DocumentRecord, 'id' | 'name'> | null, messages: ChatMessage[] = [], title?: string): AiChatTab {
  const now = new Date().toISOString();
  const documentId = doc?.id || 'general';
  const documentName = doc?.name ? displayName(doc.name) : 'General chat';
  return {
    id: makeId(),
    documentId,
    documentName,
    title: title || documentName,
    messages: normaliseAiChatMessages(messages),
    createdAt: now,
    updatedAt: now
  };
}

function normaliseAiChatTab(input: unknown): AiChatTab | null {
  if (!input || typeof input !== 'object') return null;
  const tab = input as Partial<AiChatTab>;
  const messages = normaliseAiChatMessages(tab.messages);
  const now = new Date().toISOString();
  const documentId = typeof tab.documentId === 'string' && tab.documentId.trim() ? tab.documentId.trim() : 'general';
  const documentName = typeof tab.documentName === 'string' && tab.documentName.trim() ? tab.documentName.trim().slice(0, 120) : 'General chat';
  const title = typeof tab.title === 'string' && tab.title.trim() ? tab.title.trim().slice(0, 120) : documentName;
  return {
    id: typeof tab.id === 'string' && tab.id.trim() ? tab.id : makeId(),
    documentId,
    documentName,
    title,
    messages,
...
function readAiChatTabs(): AiChatTab[] {
  if (typeof window === 'undefined') return [];
  try {
    const parsed = JSON.parse(window.localStorage.getItem(AI_CHAT_TABS_STORAGE_KEY) || '[]');
    if (Array.isArray(parsed)) {
      const tabs = parsed
        .map(normaliseAiChatTab)
        .filter((tab): tab is AiChatTab => Boolean(tab))
        .slice(-AI_CHAT_TAB_LIMIT);
      if (tabs.length) return tabs;
    }
  } catch {
    // Legacy chat memory below handles stale or missing tab storage.
  }

  const legacyMessages = readAiChatMemory();
  return legacyMessages.length ? [createAiChatTabForDocument(null, legacyMessages, 'Previous chat')] : [];
}

function readActiveAiChatTabId() {
  if (typeof window === 'undefined') return '';
  return window.localStorage.getItem(AI_CHAT_ACTIVE_TAB_STORAGE_KEY) || '';
}

function saveAiChatTabs(tabs: AiChatTab[]) {
  if (typeof window === 'undefined') return;
  const storedTabs = tabs.slice(-AI_CHAT_TAB_LIMIT).map((tab) => ({
    ...tab,
    messages: normaliseAiChatMessages(tab.messages)
  }));
  window.localStorage.setItem(AI_CHAT_TABS_STORAGE_KEY, JSON.stringify(storedTabs));
}

function saveActiveAiChatTabId(id: string) {
  if (typeof window === 'undefined') return;
  if (id) window.localStorage.setItem(AI_CHAT_ACTIVE_TAB_STORAGE_KEY, id);
  else window.localStorage.removeItem(AI_CHAT_ACTIVE_TAB_STORAGE_KEY);
}

function getDefaultAiChatFrame(): AiChatFrame {
  if (typeof window === 'undefined') {
    return { left: 24, top: 120, width: 390, height: 520 };
  }
  const width = clamp(Math.min(440, Math.max(340, window.innerWidth * 0.28)), AI_CHAT_MIN_WIDTH, Math.max(AI_CHAT_MIN_WIDTH, window.innerWidth - AI_CHAT_MARGIN * 2));
  const height = clamp(Math.min(560, Math.max(380, window.innerHeight * 0.55)), AI_CHAT_MIN_HEIGHT, Math.max(AI_CHAT_MIN_HEIGHT, window.innerHeight - AI_CHAT_MARGIN * 2));
  return clampAiChatFrame({
    left: window.innerWidth - width - 84,
    top: window.innerHeight - height - 92,
    width,
    height
  });
}

function clampAiChatFrame(frame: AiChatFrame): AiChatFrame {
  if (typeof window === 'undefined') return frame;
  const maxWidth = Math.max(AI_CHAT_MIN_WIDTH, window.innerWidth - AI_CHAT_MARGIN * 2);
  const maxHeight = Math.max(AI_CHAT_MIN_HEIGHT, window.innerHeight - AI_CHAT_MARGIN * 2);
  const width = clamp(frame.width, AI_CHAT_MIN_WIDTH, maxWidth);
  const height = clamp(frame.height, AI_CHAT_MIN_HEIGHT, maxHeight);
  const maxLeft = Math.max(AI_CHAT_MARGIN, window.innerWidth - width - AI_CHAT_MARGIN);
  const maxTop = Math.max(AI_CHAT_MARGIN, window.innerHeight - height - AI_CHAT_MARGIN);
  return {
    left: clamp(frame.left, AI_CHAT_MARGIN, maxLeft),
    top: clamp(frame.top, AI_CHAT_MARGIN, maxTop),
    width,
    height
  };
}

function readAiChatFrame(): AiChatFrame {
  if (typeof window === 'undefined') return getDefaultAiChatFrame();
  try {
    const parsed = JSON.parse(window.localStorage.getItem(AI_CHAT_FRAME_STORAGE_KEY) || 'null');
    if (
      parsed &&
      ...
      return clampAiChatFrame(parsed);
    }
  } catch {
    // Ignore stale frame data and return the default floating position.
  }
  return getDefaultAiChatFrame();
}

function readAiChatZoomIndex() {
  if (typeof window === 'undefined') return 1;
  const parsed = Number(window.localStorage.getItem(AI_CHAT_ZOOM_STORAGE_KEY));
  if (!Number.isFinite(parsed)) return 1;
  return clamp(Math.round(parsed), 0, AI_CHAT_ZOOM_LEVELS.length - 1);
}

function saveAiChatFrame(frame: AiChatFrame) {
  if (typeof window === 'undefined') return;
  window.localStorage.setItem(AI_CHAT_FRAME_STORAGE_KEY, JSON.stringify(frame));
}

function saveAiChatZoomIndex(index: number) {
  if (typeof window === 'undefined') return;
  window.localStorage.setItem(AI_CHAT_ZOOM_STORAGE_KEY, String(clamp(Math.round(index), 0, AI_CHAT_ZOOM_LEVELS.length - 1)));
}
...
async function requestDuxAiAnswer(messages: ChatMessage[], context: Record<string, unknown>): Promise<DuxAiChatAnswer> {
  ...
  try {
    ...
    let data: { answer?: string; error?: string; questionMemory?: string } = {};
    try {
    ...
    return {
      ...
      questionMemory: normaliseAiQuestionMemory(data.questionMemory) || undefined
    };
  } finally {
...
const MAX_STUDY_UPLOAD_CHARS = 14000;
const MAX_AI_IMAGE_DATA_URL_CHARS = 3200000;

function canvasToCompressedImageDataUrl(source: HTMLCanvasElement, maxSide = 1800, quality = 0.84) {
  const sourceMaxSide = Math.max(source.width, source.height);
  const ratio = sourceMaxSide > maxSide ? maxSide / sourceMaxSide : 1;
  const canvas = document.createElement('canvas');
  ...
  const attempts = [
    { side: maxSide, quality },
    { side: 1600, quality: 0.78 },
    { side: 1400, quality: 0.72 },
    { side: 1200, quality: 0.66 }
  ];

  for (const attempt of attempts) {
    const attemptRatio = Math.max(source.width, source.height) > attempt.side ? attempt.side / Math.max(source.width, source.height) : 1;
    if (attemptRatio !== ratio) {
      canvas.width = Math.max(1, Math.round(source.width * attemptRatio));
      canvas.height = Math.max(1, Math.round(source.height * attemptRatio));
      context.fillStyle = '#ffffff';
      context.fillRect(0, 0, canvas.width, canvas.height);
      context.drawImage(source, 0, 0, canvas.width, canvas.height);
    }
    const dataUrl = canvas.toDataURL('image/jpeg', attempt.quality);
    if (dataUrl.length <= MAX_AI_IMAGE_DATA_URL_CHARS) return dataUrl;
  }
...
async function imageFileToCompressedDataUrl(file: File) {
  ...
  const context = canvas.getContext('2d');
  if (!context) throw new Error('Could not prepare image for Dux AI.');
  context.fillStyle = '#ffffff';
  context.fillRect(0, 0, canvas.width, canvas.height);
  context.drawImage(image, 0, 0);
  return canvasToCompressedImageDataUrl(canvas, 1800, 0.84);
}

async function extractPdfTextFromFile(file: File, maxPages = 16) {
  const bytes = new Uint8Array(await file.arrayBuffer());
  const pdf = await pdfjsLib.getDocument({ data: bytes.buffer.slice(0) }).promise;
  const pages: string[] = [];
  ...
  return pages.join('\n\n').slice(0, MAX_STUDY_UPLOAD_CHARS);
}

async function extractTextFromStudyUpload(file: File) {
  const lowerName = file.name.toLowerCase();
  if (file.type.startsWith('image/') || /\.(png|jpe?g|webp)$/i.test(lowerName)) {
    return { text: '', imageDataUrl: await imageFileToCompressedDataUrl(file) };
  }
  if (file.type === 'application/pdf' || lowerName.endsWith('.pdf')) {
    return { text: await extractPdfTextFromFile(file), imageDataUrl: '' };
  }
  if (
    file.type.startsWith('text/') ||
    /\.(txt|csv|tsv|ics|json|md|markdown|log)$/i.test(lowerName)
  ) {
    return { text: (await file.text()).slice(0, MAX_STUDY_UPLOAD_CHARS), imageDataUrl: '' };
  }
  const text = await file.text().catch(() => '');
  return { text: text.replace(/\0/g, '').trim().slice(0, MAX_STUDY_UPLOAD_CHARS), imageDataUrl: '' };
}

function buildSchedulePromptWithUpload(prompt: string, fileName: string, fileText: string, hasImage = false) {
  const cleanPrompt = prompt.trim();
  const cleanText = fileText.trim();
  if (!cleanText && !hasImage) return cleanPrompt;
  if (!cleanText && hasImage) {
    return `${cleanPrompt || 'Create a study schedule from the uploaded timetable screenshot and study requirements.'}

Uploaded timetable/study image attached (${fileName || 'attached screenshot'}).

Read the image carefully for dates, subjects, tasks, deadlines, availability and constraints, then plan around it.`;
  }
  return `${cleanPrompt || 'Create a study schedule from the uploaded timetable and study requirements.'}

Uploaded timetable or study file (${fileName || 'attached file'}):
${cleanText}

Use the uploaded file as source context for dates, availability, subjects, deadlines, exams and constraints.`;
}


function tidyAiMathText(value: string) {
  const cleanMathChunk = (raw: string) => raw
    .replace(/\\\\+/g, ', ')
    .replace(/\s+\\\s+/g, ', ')
  ...
  const formatPlainMatrixBody = (body: string) => {
    const rows = body
      .replace(/\\\\+/g, ' | ')
      .replace(/\s+\\\s+/g, ' | ')
      .replace(/&/g, ', ')
      .split(/\s*\|\s*/)
      .map((row) => cleanMathChunk(row))
      .filter(Boolean);
    return rows.length ? `(${rows.join(', ')})` : '';
  };

  const formatLatexMatrixBody = (body: string) => {
    const rows = body
      .replace(/&/g, ', ')
      .split(/\\\\+/)
      .map((row) => cleanMathChunk(row))
...
function isAiFormulaLine(line: string) {
  const text = tidyAiMathText(line);
  if (text.length < 4 || text.length > 220) return false;
  const mathSignals = [
    /=/.test(text),
    /[∫Σ√πθλΔ±≤≥≈∞]/.test(text),
    /\b(?:sin|cos|tan|lim|dy\/dx|d\/dx)\b/i.test(text),
    /[a-zA-Z]\s*[²³]/.test(text),
    /\([^)]*,\s*[^)]*(?:,\s*[^)]*)?\)/.test(text)
...
function isAiWorkingLine(line: string) {
  const text = tidyAiMathText(line);
  return text.length <= 260 && (
    /\b(?:substitut|expand|simplif|solve|therefore|hence|differentiat|integrat|midpoint|gradient|marks?|criteria)\b/i.test(text) ||
    isAiFormulaLine(text)
  );
}
...
function AiMessageText({ text }: { text: string }) {
  ...
  lines.forEach((rawLine, index) => {
    ...
    const plainHeading = line.match(/^(Step\s*\d+|Direct answer|Method|Working|Final answer|Answer|Conclusion|Mark|Mark breakdown|Visible student answer\/working|Correct answer|Ideal response|What lost marks|HSC-standard fix|Strengths|Missing marks)\s*:?\s*(.*)$/i);
    if (plainHeading) {
      rendered.push(
        <div key={`plain-heading-${index}`} className="ai-message-heading">
          {renderInlineAiText(`${plainHeading[1]}${plainHeading[2] ? `: ${plainHeading[2]}` : ''}`, `plain-heading-${index}`)}
        </div>
      );
      return;
    }
    ...
    if (isAiFormulaLine(line)) {
      rendered.push(<div key={`formula-${index}`} className="ai-message-formula">{renderInlineAiText(line, `formula-${index}`)}</div>);
      return;
    }

    if (isAiWorkingLine(line)) {
      rendered.push(<p key={`working-${index}`} className="ai-message-working">{renderInlineAiText(line, `working-${index}`)}</p>);
      return;
    }
...
function looksLikeGreetingOnly(question: string) {
  return /^(hi|hii|hy|hey|hello|yo|sup|what'?s up|how are you|how r u|how are u|how you going|how's it going|good morning|good afternoon|good evening|gday|g'day)(?:\s+dux)?[!.?\s]*$/i.test(question.trim());
}
...
function buildMarkingFeedback(question: string): string {
  ...
  return [
    ...
    '',
    `Indicative HSC mark: ${wordCount >= 80 && hasEvidence && hasStructure ? 'high range for the marks available, if it answers the exact question' : wordCount >= 45 ? 'middle range, because the idea is present but evidence and links need work' : 'low range unless this is only a one-mark definition'}.`,
    '',
    'Mark breakdown:',
    '- Criteria: directly answers the question and command word. Evidence seen: partial. Marks: depends on the question, but do not award full marks unless the answer stays focused.',
    `- Criteria: uses evidence, equation, quote, data or example. Evidence seen: ${hasEvidence ? 'present' : 'missing or unclear'}. Marks: ${hasEvidence ? 'some credit' : 'little to no credit for evidence'}.`,
    `- Criteria: explains reasoning clearly. Evidence seen: ${hasStructure ? 'some logical linking language' : 'not enough visible reasoning'}. Marks: ${hasStructure ? 'method/explanation credit possible' : 'lost explanation marks'}.`,
    `- Criteria: links back to the question. Evidence seen: ${hasQuestionLink ? 'some link to the task' : 'weak link to the exact question'}. Marks: ${hasQuestionLink ? 'relevance credit possible' : 'lost relevance marks'}.`,
    '',
    'What works:',
...
function bytesToLocalNotesPayload(bytes: Uint8Array): ArrayBuffer {
  return bytesToTransferableBuffer(bytes);
}
...
function getCanvasPointFromClient(clientX: number, clientY: number, canvas: HTMLCanvasElement, zoom: number, pressure = 0.5): Point {
  ...
  return getCanvasPointFromRect(clientX, clientY, rect, zoom, pressure);
}

function getCanvasPointFromRect(clientX: number, clientY: number, rect: DOMRect, zoom: number, pressure = 0.5): Point {
  return {
...
function drawLiveStrokeFast(context: CanvasRenderingContext2D, stroke: Stroke, points: Point[]) {
...
async function renderElementOverlayGroup(entries: RenderableElement[], size: { width: number; totalHeight: number }, scale = 2) {
  ...
  for (const entry of entries) {
    if (entry.type === 'stroke') annotations.strokes.push(entry.item);
    if (entry.type === 'shape') annotations.shapeBoxes.push(entry.item);
    if (entry.type === 'text') annotations.textBoxes.push(entry.item);
  }
  await drawRetainedElements(context, annotations, scale, { includeText: true });
  return canvas;
}
...
export default function App() {
  ...
  const aiChatFrameDragRef = useRef<{
    mode: AiChatFrameDragMode;
    pointerId: number;
    startClientX: number;
    startClientY: number;
    startFrame: AiChatFrame;
  } | null>(null);
  ...
  const activePointerCanvasRectRef = useRef<DOMRect | null>(null);
  const activePointerZoomRef = useRef(1);
  const lastInkMeaningfulMoveAtRef = useRef(0);
  const lastInkMeaningfulPointRef = useRef<Point | null>(null);
  ...
  const liveStrokeContextRef = useRef<CanvasRenderingContext2D | null>(null);
  const liveStrokeBackingScaleRef = useRef(1);
  ...
  const mathPlaneResizeFrameRef = useRef<number | null>(null);
  const mathPlaneResizePointerRef = useRef<{ clientX: number; clientY: number } | null>(null);
  ...
  const [pdfSaveFailureMessage, setPdfSaveFailureMessage] = useState('');
  ...
  const aiScheduleFileInputRef = useRef<HTMLInputElement | null>(null);
  const [aiPrompt, setAiPrompt] = useState('');
  const [aiScheduleFileName, setAiScheduleFileName] = useState('');
  const [aiScheduleFileText, setAiScheduleFileText] = useState('');
  const [aiScheduleFileImageDataUrl, setAiScheduleFileImageDataUrl] = useState('');
  ...
  const [aiChatTabs, setAiChatTabs] = useState<AiChatTab[]>(() => readAiChatTabs());
  const [activeAiChatTabId, setActiveAiChatTabId] = useState(() => readActiveAiChatTabId());
  const [aiChatFrame, setAiChatFrame] = useState<AiChatFrame>(() => readAiChatFrame());
  const [aiChatFrameActive, setAiChatFrameActive] = useState(false);
  const [aiChatZoomIndex, setAiChatZoomIndex] = useState(() => readAiChatZoomIndex());
  const [collapsedAiMessageIds, setCollapsedAiMessageIds] = useState<string[]>([]);
  ...
  const [toolbarTooltip, setToolbarTooltip] = useState<{ text: string; top: number; left: number } | null>(null);
  ...
  const activeAiChatTab = useMemo(() => aiChatTabs.find((tab) => tab.id === activeAiChatTabId) || aiChatTabs[0] || null, [aiChatTabs, activeAiChatTabId]);
  const aiChatMessages = activeAiChatTab?.messages || [];
  const aiChatZoom = AI_CHAT_ZOOM_LEVELS[aiChatZoomIndex] || 1;
  ...
  useEffect(() => {
    saveAiChatTabs(aiChatTabs);
  }, [aiChatTabs]);

  useEffect(() => {
    saveActiveAiChatTabId(activeAiChatTab?.id || '');
  }, [activeAiChatTab?.id]);

  useEffect(() => {
    saveAiChatZoomIndex(aiChatZoomIndex);
  }, [aiChatZoomIndex]);

  useEffect(() => {
    if (!activeAiChatTabId && aiChatTabs[0]) {
      setActiveAiChatTabId(aiChatTabs[0].id);
      return;
    }
    if (activeAiChatTabId && !aiChatTabs.some((tab) => tab.id === activeAiChatTabId)) {
      setActiveAiChatTabId(aiChatTabs[0]?.id || '');
    }
  }, [activeAiChatTabId, aiChatTabs]);
  ...
  }, [aiChatOpen, activeAiChatTab?.id, aiChatMessages.length, aiChatTyping]);

  useEffect(() => {
    function clampFloatingAiFrame() {
      setAiChatFrame((current) => {
        const next = clampAiChatFrame(current);
        saveAiChatFrame(next);
        return next;
      });
    }
    window.addEventListener('resize', clampFloatingAiFrame);
    return () => window.removeEventListener('resize', clampFloatingAiFrame);
  }, []);

  function isAiChatDragBlockedTarget(target: HTMLElement | null) {
    if (!target) return false;
    return Boolean(target.closest('button, input, textarea, select, a, [contenteditable="true"], .ai-chat-tab-strip, .ai-chat-input-row, .ai-answer-actions, .ai-chat-resize-handle'));
  }

  function resizeAiChatFrameFromDrag(startFrame: AiChatFrame, dx: number, dy: number, mode: AiChatFrameDragMode) {
    let left = startFrame.left;
    let top = startFrame.top;
    let width = startFrame.width;
    let height = startFrame.height;
    const resizingWest = mode === 'resize-nw' || mode === 'resize-sw';
    const resizingNorth = mode === 'resize-nw' || mode === 'resize-ne';
    const resizingEast = mode === 'resize-ne' || mode === 'resize-se';
    const resizingSouth = mode === 'resize-sw' || mode === 'resize-se';

    if (resizingWest) {
      left = startFrame.left + dx;
      width = startFrame.width - dx;
    }
    if (resizingEast) width = startFrame.width + dx;
    if (resizingNorth) {
      top = startFrame.top + dy;
      height = startFrame.height - dy;
    }
    if (resizingSouth) height = startFrame.height + dy;

    if (width < AI_CHAT_MIN_WIDTH) {
      if (resizingWest) left = startFrame.left + startFrame.width - AI_CHAT_MIN_WIDTH;
      width = AI_CHAT_MIN_WIDTH;
    }
    if (height < AI_CHAT_MIN_HEIGHT) {
      if (resizingNorth) top = startFrame.top + startFrame.height - AI_CHAT_MIN_HEIGHT;
      height = AI_CHAT_MIN_HEIGHT;
    }

    return clampAiChatFrame({ left, top, width, height });
  }

  async function copyAiAnswer(message: ChatMessage) {
    try {
      await navigator.clipboard.writeText(message.text);
      setToastMessage('AI answer copied.');
    } catch {
      setToastMessage('Could not copy automatically.');
    }
  }

  function exportAiAnswer(message: ChatMessage) {
    const stamp = new Date(message.createdAt || Date.now()).toISOString().replace(/[:.]/g, '-');
    const blob = new Blob([message.text], { type: 'text/plain;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `dux-ai-answer-${stamp}.txt`;
    document.body.appendChild(link);
    link.click();
    link.remove();
    window.setTimeout(() => URL.revokeObjectURL(url), 1500);
    setToastMessage('AI answer exported.');
  }

  function toggleAiAnswerCollapsed(messageId: string) {
    setCollapsedAiMessageIds((ids) => ids.includes(messageId)
      ? ids.filter((id) => id !== messageId)
      : [...ids, messageId]);
  }

  function handleAiChatFramePointerDown(event: ReactPointerEvent<HTMLElement>, mode: AiChatFrameDragMode) {
    if (event.button !== 0) return;
    const target = event.target as HTMLElement | null;
    if (mode === 'move' && isAiChatDragBlockedTarget(target)) return;
    event.preventDefault();
    event.stopPropagation();
    aiChatFrameDragRef.current = {
      mode,
    ...
    setAiChatFrameActive(true);
    (aiChatPanelRef.current || event.currentTarget).setPointerCapture(event.pointerId);
  }

  function handleAiChatFramePointerMove(event: ReactPointerEvent<HTMLDivElement>) {
    const drag = aiChatFrameDragRef.current;
    ...
    const next = drag.mode === 'move'
      ? {
          ...drag.startFrame,
          left: drag.startFrame.left + dx,
          top: drag.startFrame.top + dy
        }
      : resizeAiChatFrameFromDrag(drag.startFrame, dx, dy, drag.mode);
    setAiChatFrame(clampAiChatFrame(next));
  }

  function handleAiChatFramePointerEnd(event: ReactPointerEvent<HTMLDivElement>) {
    const drag = aiChatFrameDragRef.current;
    if (!drag || drag.pointerId !== event.pointerId) return;
    try {
      (aiChatPanelRef.current || event.currentTarget).releasePointerCapture(event.pointerId);
    } catch {
      // Pointer capture can already be gone if the browser cancelled the gesture.
    }
    aiChatFrameDragRef.current = null;
    setAiChatFrameActive(false);
    setAiChatFrame((current) => {
      const next = clampAiChatFrame(current);
      saveAiChatFrame(next);
      return next;
  ...
  function updateAiChatTabMessages(tabId: string, updater: (messages: ChatMessage[]) => ChatMessage[]) {
    setAiChatTabs((tabs) => tabs.map((tab) => {
      if (tab.id !== tabId) return tab;
      const messages = normaliseAiChatMessages(updater(tab.messages));
      return { ...tab, messages, updatedAt: new Date().toISOString() };
    }));
  }

  function createNewAiChatTabForCurrentDocument() {
    const tab = createAiChatTabForDocument(activeDocRef.current);
    setAiChatTabs((tabs) => [...tabs, tab].slice(-AI_CHAT_TAB_LIMIT));
    setActiveAiChatTabId(tab.id);
    setAiChatOpen(true);
  }

  function ensureAiChatTabForDocument(doc = activeDocRef.current) {
    const documentId = doc?.id || 'general';
    const documentName = doc?.name ? displayName(doc.name) : 'General chat';
    let resolvedTabId = '';

    setAiChatTabs((tabs) => {
      const existing = tabs.find((tab) => tab.documentId === documentId);
      if (existing) {
        resolvedTabId = existing.id;
        return tabs.map((tab) => tab.id === existing.id
          ? {
              ...tab,
              documentName,
              title: tab.title === tab.documentName || !tab.title.trim() ? documentName : tab.title,
              updatedAt: tab.updatedAt || new Date().toISOString()
            }
          : tab);
      }

      const nextTab = createAiChatTabForDocument(doc);
      resolvedTabId = nextTab.id;
      return [...tabs, nextTab].slice(-AI_CHAT_TAB_LIMIT);
    });

    if (resolvedTabId) setActiveAiChatTabId(resolvedTabId);
    return resolvedTabId;
  }

  function openAiChatForCurrentDocument() {
    setTipPopupOpen(false);
    ensureAiChatTabForDocument(activeDocRef.current);
    setAiChatOpen(true);
  }

  function toggleAiChatForCurrentDocument() {
    if (aiChatOpen) {
      setAiChatOpen(false);
      return;
    }
    openAiChatForCurrentDocument();
  }

  function deleteAiChatTab(tabId: string) {
    setAiChatTabs((tabs) => {
      const next = tabs.filter((tab) => tab.id !== tabId);
      if (next.length) {
        if (tabId === activeAiChatTab?.id) setActiveAiChatTabId(next[Math.max(0, next.length - 1)].id);
        return next;
      }
      const replacement = createAiChatTabForDocument(activeDocRef.current);
      setActiveAiChatTabId(replacement.id);
      return [replacement];
    });
  }
  ...
  const scheduleSave = useCallback((doc: DocumentRecord) => {
    ...
    saveTimerRef.current = window.setTimeout(async () => {
      try {
        ...
        console.error(error);
        setStatus('Local autosave paused. Keep writing; use Save PDF when you are ready to export.');
      }
    }, AUTO_SAVE_DELAY_MS);
  }, []);
  ...
  async function extractCurrentPdfPageText(page: NotebookPage) {
    if (page.kind !== 'pdf' || !pdfRef.current) return '';
    try {
      const pdfPage = await pdfRef.current.getPage(page.sourcePage);
      ...
      console.warn('Could not extract current PDF page text for Dux AI:', error);
      return '';
    }
  }

  function messageWantsFullPageAiContext(text: string) {
    const clean = text.toLowerCase();
    return /\b(all|every|whole|entire|full)\s+(question|questions|page|worksheet|paper|document)\b/.test(clean)
      || /\b(complete|solve|answer|mark|check)\s+(all|every)\b/.test(clean)
      || /\b(on|from)\s+the\s+(whole|entire|full)\s+page\b/.test(clean);
  }

  function messageCanUseLocalAiChatReply(text: string) {
    const clean = text.trim();
    return looksLikeGreetingOnly(clean)
      || /^(thanks|thank you|cheers|ta)[!.\s]*$/i.test(clean)
      || /^(what can you do|how can you help|what are you)[!?.\s]*$/i.test(clean);
  }

  function messageRefersToEarlierAiChat(text: string, history: ChatMessage[]) {
    if (!history.some((message) => message.role === 'assistant')) return false;
    const clean = text.toLowerCase().replace(/\s+/g, ' ').trim();
    return /\b(you|your)\s+(got|gave|answered|marked|read|solved|said)\b.{0,80}\b(wrong|incorrect|mistake|missed)\b/.test(clean)
      || /\b(that|the|this|previous|earlier|last)\s+(answer|mark|solution|working|response|question)\b.{0,80}\b(wrong|incorrect|mistake|missed|again|redo|recheck)\b/.test(clean)
      || /\b(wrong answer|try again|redo that|re-?evaluate|recheck|check again|previous answer|earlier answer|last answer|same question|the one before)\b/.test(clean)
      || /\b(?:redo|re-?do|rework|re-answer|re-solve|recheck|re-?evaluate|try again on)\s+(?:question|q)\s*#?\s*\d+\b/.test(clean)
      || /\b(?:question|q)\s*\d+\s+(?:is|was|should|wrong|incorrect|again|not)\b/.test(clean)
      || /\b(?:it|that)\s+(?:is|was|should be|should've been)\s+(?:option\s*)?[a-d]\b/.test(clean);
  }

  function messageNeedsCurrentPageAiContext(text: string) {
    const clean = text.toLowerCase().replace(/\s+/g, ' ').trim();
    return messageWantsFullPageAiContext(clean)
      || /\b(?:question|q)\s*\d+\b/.test(clean)
      || /\b(?:this|current|visible|shown|above|onscreen|on screen|zoomed|viewport)\s+(?:question|page|part|answer|working|work|diagram|table|image|pdf|document|problem)\b/.test(clean)
      || /\b(?:solve|answer|complete|mark|check|read|scan|ocr|explain)\s+(?:this|the|my|current|visible)\s+(?:question|page|answer|working|work|handwriting|diagram|table|image|pdf|problem)\b/.test(clean)
      || /\b(?:look at|use|read|scan|see)\s+(?:the|this|my|current)\s+(?:page|pdf|screen|viewport|image|question|answer|working|handwriting|diagram|table)\b/.test(clean)
      || /\b(?:my working|my answer|my handwriting|circled answer|selected answer|visible page|current page|current pdf|page image|page text|on this page|in this pdf|in the screenshot)\b/.test(clean);
  }

  function getAiChatTurnContextRoute(text: string, history: ChatMessage[]) {
    if (messageCanUseLocalAiChatReply(text)) {
      return {
        mode: 'conversation',
        note: 'The latest message is a simple conversation turn. Answer it directly without solving whatever page happens to be visible.',
        useCurrentPage: false,
        replyLocally: true
      };
    }
    if (messageRefersToEarlierAiChat(text, history)) {
      return {
        mode: 'chat-history-follow-up',
        note: 'The latest message corrects or revisits an earlier chat answer. Use the chat history and the user correction first; do not replace it with a newly visible page.',
        useCurrentPage: false,
        replyLocally: false
      };
    }
    if (messageNeedsCurrentPageAiContext(text)) {
      return {
        mode: 'current-page',
        note: 'The latest message explicitly asks about page material, visible work or a numbered question.',
        useCurrentPage: true,
        replyLocally: false
      };
    }
    return {
      mode: 'general-chat',
      note: 'The latest message is a normal chat or study question. No fresh page capture was requested.',
      useCurrentPage: false,
      replyLocally: false
    };
  }

  function buildAiQuestionMemoryContext(text: string, history: ChatMessage[]) {
    const requestedNumbers = extractAiQuestionNumbers(text);
    const memoryMessages = history
      .filter((message) => message.role === 'assistant' && Boolean(message.questionMemory))
      .map((message) => ({
        message,
        numbers: normaliseAiQuestionNumbers(message.questionNumbers).length
          ? normaliseAiQuestionNumbers(message.questionNumbers)
          : extractAiQuestionNumbers(`${message.text}\n${message.questionMemory || ''}`)
      }));
    const requestedMatches = requestedNumbers.length
      ? memoryMessages.filter(({ message, numbers }) => (
        requestedNumbers.some((number) => numbers.includes(number))
        || requestedNumbers.some((number) => new RegExp(`\\b(?:question|q)\\s*#?\\s*${number}\\b`, 'i').test(`${message.text}\n${message.questionMemory || ''}`))
      ))
      : [];
    const selected = (requestedMatches.length ? requestedMatches : memoryMessages).slice(-3);
    if (!selected.length) return '';
    return selected.map(({ message, numbers }, index) => {
      const labels = numbers.length ? `Question reference${numbers.length === 1 ? '' : 's'}: ${numbers.map((number) => `question ${number}`).join(', ')}.` : 'Question reference not numbered.';
      return `Prior page evidence ${index + 1}. ${labels}\n${message.questionMemory}`;
    }).join('\n\n').slice(0, 12000);
  }

  function buildAiPageQuestionMemory(pageContext: Record<string, unknown>) {
    const scope = typeof pageContext.currentPageImageScope === 'string' ? pageContext.currentPageImageScope : '';
    const visibleRegion = typeof pageContext.currentPageVisibleRegion === 'string' ? pageContext.currentPageVisibleRegion : '';
    const pageText = typeof pageContext.currentPageText === 'string' ? pageContext.currentPageText : '';
    const annotationText = typeof pageContext.currentPageAnnotationText === 'string' ? pageContext.currentPageAnnotationText : '';
    const lines = [
      scope ? `Page evidence scope: ${scope}` : '',
      visibleRegion ? `Visible region: ${visibleRegion}` : '',
      pageText ? `Readable page text:\n${pageText}` : '',
      annotationText ? `Typed annotations and page summary:\n${annotationText}` : ''
    ].filter(Boolean);
    return normaliseAiQuestionMemory(lines.join('\n\n'));
  }

  function getVisiblePageClip(pageKey: string, view: { width: number; totalHeight: number }) {
    const stage = canvasStageRef.current;
    const pageNode = pageShadowRefs.current[pageKey];
    if (!stage || !pageNode) return null;
    ...
    if (visibleRight - visibleLeft < 80 || visibleBottom - visibleTop < 80) return null;
    const x = clamp((visibleLeft - pageRect.left) / zoom, 0, Math.max(0, view.width - 1));
    const y = clamp((visibleTop - pageRect.top) / zoom, 0, Math.max(0, view.totalHeight - 1));
    const width = clamp((visibleRight - visibleLeft) / zoom, 1, Math.max(1, view.width - x));
    const height = clamp((visibleBottom - visibleTop) / zoom, 1, Math.max(1, view.totalHeight - y));
    return { x, y, width, height };
  }

  function extractAnnotationTextForAi(annotations: PageAnnotations, clip?: { x: number; y: number; width: number; height: number } | null) {
    const includeRect = (rect: { x: number; y: number; width: number; height: number }) => !clip || rectsIntersect(rect, clip);
    const textBoxes = annotations.textBoxes
      .filter((box) => includeRect({ x: box.x, y: box.y, width: box.width, height: Math.max(box.minHeight, 40) }))
      .map((box, index) => box.text.trim() ? `Text box ${index + 1}: ${box.text.trim()}` : '')
      .filter(Boolean);
    const shapeCount = annotations.shapeBoxes.filter((shape) => includeRect(normaliseRect(shape.x, shape.y, shape.width, shape.height))).length;
    const planeCount = annotations.mathPlaneBoxes.filter((plane) => includeRect({ x: plane.x, y: plane.y, width: plane.width, height: plane.height })).length;
    const imageCount = annotations.imageBoxes.filter((image) => includeRect({ x: image.x, y: image.y, width: image.width, height: image.height })).length;
    const strokeCount = annotations.strokes.filter((stroke) => !clip || strokeIntersectsRect(stroke, clip)).length;
    const summary = [
      strokeCount ? `${strokeCount} handwritten ink stroke${strokeCount === 1 ? '' : 's'} are visible in the supplied ${clip ? 'viewport crop' : 'page image'}. Read the handwriting visually before marking or solving.` : '',
      imageCount ? `${imageCount} inserted image element${imageCount === 1 ? '' : 's'} are visible in the supplied ${clip ? 'viewport crop' : 'page image'}.` : '',
      shapeCount ? `${shapeCount} shape element${shapeCount === 1 ? '' : 's'} are visible in the supplied ${clip ? 'viewport crop' : 'page image'}.` : '',
      planeCount ? `${planeCount} Cartesian plane element${planeCount === 1 ? '' : 's'} are visible in the supplied ${clip ? 'viewport crop' : 'page image'}.` : ''
    ].filter(Boolean);
    return [...textBoxes, ...summary].join('\n').slice(0, 5000);
  }

  async function buildCurrentPageAiContext(userIntent = '') {
    const data = getActivePageData();
    if (!data) return {};
    const useFullPage = messageWantsFullPageAiContext(userIntent) || zoom <= 1.15;
    const visibleClip = useFullPage ? null : getVisiblePageClip(data.page.key, data.view);
    const shouldScopeToViewport = Boolean(visibleClip && !useFullPage);
    const pdfPageText = shouldScopeToViewport ? '' : await extractCurrentPdfPageText(data.page);
    const annotationText = extractAnnotationTextForAi(data.annotations, visibleClip);
    let pageImageDataUrl = '';
    try {
      const { canvas } = await renderExportCanvas(data.page, data.annotations, pdfRef.current, activeTheme, 2.8);
      if (visibleClip) {
        const scaleX = canvas.width / data.view.width;
        const scaleY = canvas.height / data.view.totalHeight;
        const cropCanvas = document.createElement('canvas');
        cropCanvas.width = Math.max(1, Math.round(visibleClip.width * scaleX));
        cropCanvas.height = Math.max(1, Math.round(visibleClip.height * scaleY));
        const cropContext = cropCanvas.getContext('2d');
        if (!cropContext) throw new Error('Could not crop current viewport for Dux AI.');
        cropContext.fillStyle = '#ffffff';
        cropContext.fillRect(0, 0, cropCanvas.width, cropCanvas.height);
        cropContext.drawImage(
          canvas,
          Math.round(visibleClip.x * scaleX),
          Math.round(visibleClip.y * scaleY),
          Math.round(visibleClip.width * scaleX),
          Math.round(visibleClip.height * scaleY),
          0,
          0,
          cropCanvas.width,
          cropCanvas.height
        );
        pageImageDataUrl = canvasToCompressedImageDataUrl(cropCanvas, 2300, 0.9);
      } else {
        pageImageDataUrl = canvasToCompressedImageDataUrl(canvas, 2200, 0.88);
      }
    } catch (error) {
      console.warn('Could not prepare current page image for Dux AI:', error);
    }
    return {
      currentPageText: pdfPageText,
      currentPageAnnotationText: annotationText,
      currentPageImageDataUrl: pageImageDataUrl,
      currentPageImageScope: shouldScopeToViewport
        ? 'Visible viewport crop only. Answer the question/work that is visible in this crop unless the user explicitly asks for all questions or the whole page.'
        : 'Full current page.',
      currentPageVisibleRegion: visibleClip
        ? `x ${Math.round(visibleClip.x)}, y ${Math.round(visibleClip.y)}, width ${Math.round(visibleClip.width)}, height ${Math.round(visibleClip.height)} in page coordinates.`
        : '',
      currentPageTextScope: shouldScopeToViewport ? 'PDF text omitted because the user is zoomed in; use the attached viewport crop as the source of truth.' : 'Full current page text when extractable.',
      currentPageHasReadableText: Boolean(pdfPageText || annotationText),
      currentPageKind: data.page.kind,
      currentPageSourcePage: data.page.kind === 'pdf' ? data.page.sourcePage : null
    };
  }
  ...
  function drawLiveStrokeSegment(pageKey: string, stroke: Stroke, points: Point[]) {
    if (points.length < 2) return;
    const isCurrentLivePage = pageKey === liveStrokePageKeyRef.current;
    let context = isCurrentLivePage ? liveStrokeContextRef.current : null;
    let backingScale = isCurrentLivePage ? liveStrokeBackingScaleRef.current : 0;
    if (!context) {
      const canvas = inkCanvasRefs.current[pageKey] || (getActivePageData()?.page.key === pageKey ? inkCanvasRef.current : null);
      context = canvas?.getContext('2d') || null;
      backingScale = getPageBackingScale(pageKey);
    }
    if (!context) return;
    overlayRenderTokenRef.current += 1;
    context.save();
    context.setTransform(backingScale, 0, 0, backingScale, 0, 0);
    drawLiveStrokeFast(context, stroke, points);
    context.restore();
  }
  ...
  const renderAllPages = useCallback(async () => {
    ...
    if (imageDragRef.current || imageResizeRef.current || mathPlaneResizeRef.current || shapeResizeRef.current || selectionDragRef.current || textDragRef.current || textResizeRef.current) return;
  ...
  useEffect(() => {
    const hasUnpaintedPage = Boolean(activeDoc?.pages.some((page) => !pageViewsRef.current[page.key]));
    if (hasUnpaintedPage) {
      void renderCurrentPage();
      return;
    }

    // Pinch and trackpad zoom already scale the live page shells. Repaint the
    // PDF/page backing canvases after the gesture settles instead of doing a
    // heavy all-page render for every tiny zoom tick.
    const timer = window.setTimeout(() => {
      void renderCurrentPage();
    }, 130);
    return () => window.clearTimeout(timer);
  }, [activeDoc, renderCurrentPage]);
  ...
  useEffect(() => {
    function onKeyDown(event: KeyboardEvent) {
      ...
      if (!isTyping && (selectedElementKeys.length > 0 || selectedImageId) && (event.key === 'Delete' || event.key === 'Backspace')) {
        event.preventDefault();
        if (selectedElementKeys.length > 0) handleDeleteSelectedElements();
        else if (selectedImageId) handleDeleteSelectedImage();
        return;
      }
  ...
  function handleStudyDraftStartTimeChange(startTime: string) {
    setStudyDraft((current) => ({
      ...current,
      startTime,
      endTime: repairEndTimeAfterStart(startTime, current.endTime)
    }));
  }

  function handleStudyDraftEndTimeChange(endTime: string) {
    setStudyDraft((current) => ({
      ...current,
      endTime: repairEndTimeAfterStart(current.startTime, endTime)
    }));
  }

  function handleAiStartDateChange(startDate: string) {
    const nextStart = startDate || todayIsoDate();
    setAiStartDate(nextStart);
    setAiEndDate((currentEnd) => (!currentEnd || currentEnd < nextStart ? nextStart : currentEnd));
  }

  function handleAiEndDateChange(endDate: string) {
    const safeStart = aiStartDate || todayIsoDate();
    setAiEndDate(!endDate || endDate < safeStart ? safeStart : endDate);
  }
  ...
  function saveStudySession() {
    ...
    const item: StudyScheduleItem = {
      ...
      endTime: repairEndTimeAfterStart(studyDraft.startTime || '16:00', studyDraft.endTime || '17:00'),
  ...
  async function handleAiScheduleFile(event: ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0];
    event.target.value = '';
    if (!file) return;
    await processAiScheduleUploadFile(file);
  }

  async function processAiScheduleUploadFile(file: File) {
    setAiLoading(true);
    setAiError('');
    setAiSummary('');
    setStatus(`Reading ${file.name} for the study planner...`);
    try {
      const { text, imageDataUrl } = await extractTextFromStudyUpload(file);
      setAiScheduleFileName(file.name);
      setAiScheduleFileText(text);
      setAiScheduleFileImageDataUrl(imageDataUrl);
      if (text.trim() || imageDataUrl) {
        setStatus(`Attached ${file.name} to the study schedule generator.`);
      } else {
        setAiError('I could not find readable text in that file. Try a selectable PDF, screenshot image, CSV, TXT, ICS, JSON or Markdown file.');
        setStatus('Study schedule file did not contain readable text.');
      }
    } catch (error) {
      console.warn('Study schedule upload failed:', error);
      setAiError('I could not read that file. Try a screenshot image, text PDF, CSV, TXT or ICS file.');
      setStatus('Study schedule file upload failed.');
    } finally {
      setAiLoading(false);
    }
  }

  function handleAiScheduleFileDrop(event: ReactDragEvent<HTMLDivElement>) {
    event.preventDefault();
    event.stopPropagation();
    const file = event.dataTransfer.files?.[0];
    if (file) void processAiScheduleUploadFile(file);
  }

  function clearAiScheduleFile() {
    setAiScheduleFileName('');
    setAiScheduleFileText('');
    setAiScheduleFileImageDataUrl('');
    setAiError('');
  }
  ...
  async function generateScheduleWithAi() {
    const planningPrompt = buildSchedulePromptWithUpload(aiPrompt, aiScheduleFileName, aiScheduleFileText, Boolean(aiScheduleFileImageDataUrl));
    if (!planningPrompt.trim()) {
      setAiError('Write what you need planned or upload a timetable/study file first.');
      return;
    }
    ...
    try {
      ...
      const endDate = inferSmartEndDate(planningPrompt, aiStartDate, aiEndDate);
      if (endDate !== aiEndDate) setAiEndDate(endDate);

      const subjects = extractSmartSubjects(planningPrompt, folders);
      const windows = extractAvailabilityWindows(planningPrompt);
      const duration = extractSessionDuration(planningPrompt);
      const breakMinutes = extractBreakMinutes(planningPrompt);
      ...
      try {
        const response = await requestDuxStudySchedule({
          ...
          breakMinutes,
          imageDataUrl: aiScheduleFileImageDataUrl || undefined
        });
        ...
        for (const date = new Date(startDate); date.getTime() <= finalDate.getTime(); date.setDate(date.getDate() + 1)) {
          ...
          for (const windowItem of dayWindows) {
            ...
            while (cursor + duration <= windowEnd && localItems.length < 80) {
              ...
              const smartSession = makeOfflineStudySession(subject, subjectIndex, planningPrompt, duration, breakMinutes, flashcardDeckNames);
              localItems.push({
                ...smartSession,
  ...
  function makeStraightLineStroke(stroke: Stroke): Stroke {
    const first = stroke.points[0];
    const last = stroke.points[stroke.points.length - 1];
    return { ...stroke, points: [{ ...first }, { ...last }] };
  }

  function makeCircleStroke(stroke: Stroke, bounds: { x: number; y: number; width: number; height: number }): Stroke {
    const center = { x: bounds.x + bounds.width / 2, y: bounds.y + bounds.height / 2 };
    const radius = Math.max(4, (bounds.width + bounds.height) / 4);
    const first = stroke.points[0];
  ...
  function getHeldStrokeSnap(stroke: Stroke): { kind: 'line' | 'circle'; stroke: Stroke } | null {
    const points = stroke.points.filter((point, index) => index === 0 || distanceBetween(point, stroke.points[index - 1]) > 0.12);
    ...
    const last = points[points.length - 1];
    const endpointDistance = distanceBetween(first, last);

    if (pathLength >= 10 && endpointDistance >= 8) {
      let maxLineDeviation = 0;
      for (const point of points) {
        maxLineDeviation = Math.max(maxLineDeviation, distanceToSegment(point, first, last));
      }
      const lineEfficiency = endpointDistance / Math.max(1, pathLength);
      const slimEnough = minSide / maxSide <= 0.34;
      const closeToLine = maxLineDeviation <= Math.max(4.5, diagonal * 0.085);
      if (lineEfficiency >= 0.78 && (slimEnough || closeToLine)) {
        return { kind: 'line', stroke: makeStraightLineStroke(stroke) };
      }
    }

    if (maxSide < 18 || minSide < 12) return null;
    const aspect = width / height;
    if (aspect < 0.54 || aspect > 1.86) return null;
    if (pathLength < maxSide * 1.65) return null;
    if (endpointDistance > maxSide * 0.58) return null;

    const center = { x: bounds.x + width / 2, y: bounds.y + height / 2 };
    const radius = Math.max(1, (width + height) / 4);
    const radialErrors = points.map((point) => Math.abs(distanceBetween(point, center) - radius));
    const averageRadialError = radialErrors.reduce((sum, value) => sum + value, 0) / radialErrors.length;
    const maxRadialError = Math.max(...radialErrors);
    const sortedAngles = points
      .map((point) => Math.atan2(point.y - center.y, point.x - center.x))
      .sort((a, b) => a - b);
    let largestAngleGap = 0;
    for (let index = 1; index < sortedAngles.length; index += 1) {
      largestAngleGap = Math.max(largestAngleGap, sortedAngles[index] - sortedAngles[index - 1]);
    }
    largestAngleGap = Math.max(largestAngleGap, (sortedAngles[0] + Math.PI * 2) - sortedAngles[sortedAngles.length - 1]);

    const circleLike = averageRadialError <= radius * 0.32
      && maxRadialError <= radius * 0.72
      && largestAngleGap <= 1.75
      && pathLength / Math.max(1, diagonal) >= 1.72;
    if (!circleLike) return null;

    return { kind: 'circle', stroke: makeCircleStroke(stroke, bounds) };
  }
  ...
  function getScratchContactMetrics(candidate: Stroke, path: Point[], radius: number) {
    let intersections = 0;
    let nearSegments = 0;
    let minDistance = Number.POSITIVE_INFINITY;
    let firstNearSegment = -1;
    let lastNearSegment = -1;
    const candidateSegments = Math.max(0, candidate.points.length - 1);

    if (candidate.points.length <= 1) {
      minDistance = path.length <= 1 ? distanceBetween(candidate.points[0], path[0]) : Math.min(...path.map((point) => distanceBetween(point, candidate.points[0])));
      return {
        intersections: 0,
        nearSegments: minDistance <= radius ? 1 : 0,
        minDistance,
        nearSpread: minDistance <= radius ? 1 : 0,
        coverage: minDistance <= radius ? 1 : 0
      };
    }

    for (let candidateIndex = 1; candidateIndex < candidate.points.length; candidateIndex += 1) {
      const candidateStart = candidate.points[candidateIndex - 1];
      const candidateEnd = candidate.points[candidateIndex];
      ...
      for (let pathIndex = 1; pathIndex < path.length; pathIndex += 1) {
        ...
        if (segmentsIntersect(candidateStart, candidateEnd, pathStart, pathEnd)) {
          intersections += 1;
          minDistance = 0;
          segmentTouched = true;
          continue;
        ...
        const distance = segmentToSegmentDistance(candidateStart, candidateEnd, pathStart, pathEnd);
        if (distance < minDistance) minDistance = distance;
        if (distance <= radius) segmentTouched = true;
      }

      if (segmentTouched) {
        nearSegments += 1;
        if (firstNearSegment < 0) firstNearSegment = candidateIndex - 1;
        lastNearSegment = candidateIndex - 1;
      }
    }

    const nearSpread = firstNearSegment < 0 ? 0 : lastNearSegment - firstNearSegment + 1;
    return {
      intersections,
      nearSegments,
      minDistance,
      nearSpread,
      coverage: candidateSegments ? nearSpread / candidateSegments : 0
    };
  }

  function scratchPathTouchesStroke(candidate: Stroke, path: Point[], scribbleBounds: { x: number; y: number; width: number; height: number }, scratchStrokeWidth: number) {
    if (!candidate.points.length || path.length < 2) return false;
    const visualContactRadius = Math.min(6.5, Math.max(2.25, (scratchStrokeWidth || 1) * 0.65 + (candidate.width || 1) * 0.55 + 1));
    const candidateBounds = getStrokeBounds(candidate);
    if (!boundsOverlap(scribbleBounds, candidateBounds, visualContactRadius)) return false;

    const metrics = getScratchContactMetrics(candidate, path, visualContactRadius);
    if (metrics.intersections > 0) return true;

    const veryClose = metrics.minDistance <= Math.max(1.15, visualContactRadius * 0.38);
    const enoughTouchedSegments = metrics.nearSegments >= Math.min(3, Math.max(1, candidate.points.length - 1));
    return veryClose && enoughTouchedSegments && metrics.coverage >= 0.08;
  }

  function getScribbleEraseTargetIds(stroke: Stroke, existingStrokes: Stroke[]): string[] {
    if (stroke.tool !== 'pen') return [];
    if (getHeldStrokeSnap(stroke)) return [];
    const path = stroke.points.filter((point, index) => index === 0 || distanceBetween(point, stroke.points[index - 1]) > 0.08);
    ...
    const looksLikeScratchOut = pathLength > 30 * zoomAdjustedThresholdScale
      && diagonal > 6 * zoomAdjustedThresholdScale
      && Math.max(width, height) > 5 * zoomAdjustedThresholdScale
      && Math.min(width, height) > 1.6 * zoomAdjustedThresholdScale
      && compactness > 1.55
      && directionChanges >= 1;

    if (!looksLikeScratchOut) return [];

    const expandedScribbleBounds = {
      x: bounds.x,
      y: bounds.y,
      width,
      height
    ...
    const targetIds = existingStrokes
      ...
      .filter((candidate) => scratchPathTouchesStroke(candidate, path, expandedScribbleBounds, stroke.width))
  ...
  function applyMathPlaneResizePreview(clientX: number, clientY: number) {
    const resize = mathPlaneResizeRef.current;
    if (!resize) return;
    const dx = (clientX - resize.startClientX) / zoom;
    const dy = (clientY - resize.startClientY) / zoom;
    const next = normaliseDocument(cloneDocument(resize.originalDoc), activeThemeId);
    const annotations = getPageAnnotations(next, resize.pageKey);
    const bounds = pageViewsRef.current[resize.pageKey] || pageView;
    ...
    const nextAnnotations = { ...annotations, mathPlaneBoxes };
    setPageAnnotations(next, resize.pageKey, nextAnnotations);
    activeDocRef.current = next;
    setActiveDoc(next);
    void redrawPageOverlay(resize.pageKey, nextAnnotations);
  }
  ...
  function handleMathPlaneResizeMove(event: ReactPointerEvent<HTMLButtonElement>) {
    ...
    event.preventDefault();
    event.stopPropagation();
    mathPlaneResizePointerRef.current = { clientX: event.clientX, clientY: event.clientY };
    if (mathPlaneResizeFrameRef.current !== null) return;
    mathPlaneResizeFrameRef.current = window.requestAnimationFrame(() => {
      mathPlaneResizeFrameRef.current = null;
      const pointer = mathPlaneResizePointerRef.current;
      if (!pointer) return;
      applyMathPlaneResizePreview(pointer.clientX, pointer.clientY);
    });
  }
  ...
  function handleMathPlaneResizeEnd(event: ReactPointerEvent<HTMLButtonElement>) {
    event.preventDefault();
    event.stopPropagation();
    if (mathPlaneResizeFrameRef.current !== null) {
      window.cancelAnimationFrame(mathPlaneResizeFrameRef.current);
      mathPlaneResizeFrameRef.current = null;
    }
    const pendingPointer = mathPlaneResizePointerRef.current;
    if (pendingPointer) applyMathPlaneResizePreview(pendingPointer.clientX, pendingPointer.clientY);
    mathPlaneResizePointerRef.current = null;
    const current = activeDocRef.current;
  ...
  function getStageZoomAnchor(clientX?: number, clientY?: number, baseZoom = zoom): ZoomAnchor | null {
    const stage = canvasStageRef.current;
    if (!stage) return null;
    const rect = stage.getBoundingClientRect();
    const stageX = clamp((typeof clientX === 'number' ? clientX : rect.left + rect.width / 2) - rect.left, 0, rect.width);
    const stageY = clamp((typeof clientY === 'number' ? clientY : rect.top + rect.height / 2) - rect.top, 0, rect.height);
    const anchorClientX = rect.left + stageX;
    const anchorClientY = rect.top + stageY;
    let bestPageKey = '';
    let bestPageRect: DOMRect | null = null;
    let bestDistance = Number.POSITIVE_INFINITY;
    for (const page of activeDocRef.current?.pages || []) {
      const pageNode = pageShadowRefs.current[page.key];
      if (!pageNode) continue;
      const pageRect = pageNode.getBoundingClientRect();
      const inside = anchorClientX >= pageRect.left && anchorClientX <= pageRect.right && anchorClientY >= pageRect.top && anchorClientY <= pageRect.bottom;
      const horizontalDistance = anchorClientX < pageRect.left ? pageRect.left - anchorClientX : anchorClientX > pageRect.right ? anchorClientX - pageRect.right : 0;
      const verticalDistance = anchorClientY < pageRect.top ? pageRect.top - anchorClientY : anchorClientY > pageRect.bottom ? anchorClientY - pageRect.bottom : 0;
      const distance = inside ? -1 : Math.hypot(horizontalDistance, verticalDistance);
      if (distance < bestDistance) {
        bestDistance = distance;
        bestPageKey = page.key;
        bestPageRect = pageRect;
      }
      if (inside) break;
    }

    if (bestPageKey && bestPageRect) {
      return {
        pageKey: bestPageKey,
        pageX: clamp((anchorClientX - bestPageRect.left) / baseZoom, 0, Math.max(0, bestPageRect.width / baseZoom)),
        pageY: clamp((anchorClientY - bestPageRect.top) / baseZoom, 0, Math.max(0, bestPageRect.height / baseZoom)),
        contentX: (stage.scrollLeft + stageX) / baseZoom,
        contentY: (stage.scrollTop + stageY) / baseZoom,
        stageX,
        stageY
      };
    }
  ...
  function getLastStagePointerZoomAnchor() {
    const stage = canvasStageRef.current;
    const pointer = lastStagePointerRef.current;
    if (!stage || !pointer) return getStageZoomAnchor();
    const rect = stage.getBoundingClientRect();
    const insideStage = pointer.clientX >= rect.left && pointer.clientX <= rect.right && pointer.clientY >= rect.top && pointer.clientY <= rect.bottom;
    return insideStage ? getStageZoomAnchor(pointer.clientX, pointer.clientY) : getStageZoomAnchor();
  }

  function rememberStagePointer(event: ReactPointerEvent<HTMLElement>) {
    lastStagePointerRef.current = { clientX: event.clientX, clientY: event.clientY };
  }

  function scheduleZoomChange(nextZoom: number, anchor: ZoomAnchor | null = null) {
    pendingZoomAnchorRef.current = anchor;
    pendingZoomRef.current = clamp(nextZoom, MIN_PAGE_ZOOM, MAX_PAGE_ZOOM);
    ...
    zoomFrameRef.current = window.requestAnimationFrame(() => {
      ...
      setZoom((current) => Math.abs(current - pending) < 0.001 ? current : pending);
    });
  }
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      ...
      const nextStage = canvasStageRef.current;
      if (!nextStage) return;
      const previousScrollBehavior = nextStage.style.scrollBehavior;
      nextStage.style.scrollBehavior = 'auto';
      if (anchor.pageKey && typeof anchor.pageX === 'number' && typeof anchor.pageY === 'number') {
        const pageNode = pageShadowRefs.current[anchor.pageKey];
        const stageRect = nextStage.getBoundingClientRect();
        const pageRect = pageNode?.getBoundingClientRect();
        if (pageRect) {
          const anchoredClientX = pageRect.left + anchor.pageX * zoom;
          const anchoredClientY = pageRect.top + anchor.pageY * zoom;
          nextStage.scrollLeft = Math.max(0, nextStage.scrollLeft + anchoredClientX - (stageRect.left + anchor.stageX));
          nextStage.scrollTop = Math.max(0, nextStage.scrollTop + anchoredClientY - (stageRect.top + anchor.stageY));
        } else {
          nextStage.scrollLeft = Math.max(0, anchor.contentX * zoom - anchor.stageX);
          nextStage.scrollTop = Math.max(0, anchor.contentY * zoom - anchor.stageY);
        }
      } else {
        nextStage.scrollLeft = Math.max(0, anchor.contentX * zoom - anchor.stageX);
        nextStage.scrollTop = Math.max(0, anchor.contentY * zoom - anchor.stageY);
      }
      window.requestAnimationFrame(() => { nextStage.style.scrollBehavior = previousScrollBehavior; });
      });
    });
    return () => {
      window.cancelAnimationFrame(frame);
  ...
  function scheduleStagePinchZoom(center: { clientX: number; clientY: number }, distance: number) {
    ...
    scheduleZoomChange(nextZoom, getStageZoomAnchor(center.clientX, center.clientY, zoom) || {
      contentX: (pan.scrollLeft + centerXInStage) / pan.startZoom,
      contentY: (pan.scrollTop + centerYInStage) / pan.startZoom,
      stageX: centerXInStage,
      stageY: centerYInStage
    });
  }
  ...
  function zoomStageAtClientPoint(clientX: number, clientY: number, deltaY: number) {
    ...
    scheduleZoomChange(nextZoom, getStageZoomAnchor(clientX, clientY, currentZoom));
  }
  ...
  function handleCanvasStageTouchMove(event: ReactTouchEvent<HTMLElement>) {
    if (event.touches.length >= 2 && !hasActiveInkGesture()) {
      ...
      if (pan && center) {
        ...
        const shouldPinch = pan.mode === 'pinch' || (distanceDelta > 18 && distanceRatio > 0.03 && distanceDelta > centroidTravel * 0.55);
        if (shouldPinch) {
  ...
  function handlePointerDown(event: ReactPointerEvent<HTMLCanvasElement>, indexOverride?: number) {
    ...
    activePointerCanvasRectRef.current = canvas.getBoundingClientRect();
    activePointerZoomRef.current = zoom;
    const annotations = getPageAnnotations(doc, page.key);
    ...
    setIsInking(true);
    capturePointerSafely(event.currentTarget, event.pointerId);
    lastInkMeaningfulMoveAtRef.current = performance.now();
    lastInkMeaningfulPointRef.current = point;
    const stroke: Stroke = {
      id: makeId(),
    ...
    const liveCanvas = inkCanvasRefs.current[page.key] || canvas;
    liveStrokeContextRef.current = liveCanvas.getContext('2d');
    liveStrokeBackingScaleRef.current = getPageBackingScale(page.key);
    overlayRenderTokenRef.current += 1;
  }
  ...
  function handlePointerMove(event: ReactPointerEvent<HTMLCanvasElement>, indexOverride?: number) {
    ...
    const keepDistance = currentStrokeRef.current.tool === 'highlighter' ? 0.18 : 0.12;
    for (const nextPoint of points) {
      if (shouldKeepStrokePoint(currentStrokeRef.current.points, nextPoint, keepDistance)) {
        const lastMeaningfulPoint = lastInkMeaningfulPointRef.current;
        if (!lastMeaningfulPoint || distanceBetween(lastMeaningfulPoint, nextPoint) >= SCRIBBLE_ERASE_HOLD_MOVE_TOLERANCE) {
          lastInkMeaningfulMoveAtRef.current = performance.now();
          lastInkMeaningfulPointRef.current = nextPoint;
        }
  ...
  function handlePointerUp(event?: ReactPointerEvent<HTMLCanvasElement>) {
    ...
    setIsInking(false);
    activePointerCanvasRectRef.current = null;
    activePointerZoomRef.current = zoom;
    touchPanRef.current = null;
    ...
    liveStrokeContextRef.current = null;
    liveStrokeBackingScaleRef.current = 1;
    liveStrokeDrawnPointCountRef.current = 0;
    currentDraftPageKeyRef.current = null;

    if (finishedStroke.points.length < 2) {
      lastInkMeaningfulPointRef.current = null;
      lastInkMeaningfulMoveAtRef.current = 0;
      void redrawPageOverlay(page.key, annotations);
      return;
    }
    const savedStroke = { ...finishedStroke, points: smoothStrokePoints(finishedStroke.points, finishedStroke.tool) };
    const heldStillAtEnd = performance.now() - lastInkMeaningfulMoveAtRef.current >= SCRIBBLE_ERASE_HOLD_MS;
    const scratchEraseTargets = heldStillAtEnd ? getScribbleEraseTargetIds(savedStroke, annotations.strokes) : [];
    lastInkMeaningfulPointRef.current = null;
    lastInkMeaningfulMoveAtRef.current = 0;
    if (scratchEraseTargets.length) {
      ...
      setStatus(`Scribble-hold erased ${scratchEraseTargets.length} stroke${scratchEraseTargets.length === 1 ? '' : 's'} after a 0.33s pause.`);
      return;
    }

    const heldSnap = heldStillAtEnd ? getHeldStrokeSnap(savedStroke) : null;
    const finalStroke = heldSnap?.stroke || savedStroke;

    applyDocumentUpdate((draft) => {
      const nextAnnotations = getPageAnnotations(draft, page.key);
      setPageAnnotations(draft, page.key, { ...nextAnnotations, strokes: [...nextAnnotations.strokes, finalStroke] });
      return draft;
    });
    if (heldSnap?.kind === 'line') setStatus('Stroke snapped into a straight line.');
    if (heldSnap?.kind === 'circle') setStatus('Stroke snapped into a perfect circle.');
  }
  ...
  async function buildEditableSidecar(docAtExportStart: DocumentRecord) {
    const cleanDoc = cloneDocument(docAtExportStart);
    cleanDoc.thumbnailDataUrl = null;
    let originalPdfBase64: string | null = null;
    const desktopAppWillAttachOriginalPdf = window.localNotes?.runtime === 'electron';
    if (cleanDoc.pdfFileName && !desktopAppWillAttachOriginalPdf) {
      try {
    ...
    return {
      ...
      document: cleanDoc,
      originalPdfAttachedByDesktopApp: desktopAppWillAttachOriginalPdf && Boolean(cleanDoc.pdfFileName),
      originalPdfBase64
    };
  }

  function canCopyOriginalPdfPageForExport(page: NotebookPage): page is Extract<NotebookPage, { kind: 'pdf' }> {
    if (page.kind !== 'pdf') return false;
    const crop = page.crop || { top: 0, right: 0, bottom: 0, left: 0 };
  ...
  async function buildCurrentPdfExport(docAtExportStart: DocumentRecord) {
    ...
    let sourcePdfExportDoc: PDFDocument | null = null;
    if (docAtExportStart.pdfFileName) {
      try {
        const originalPdfBytes = new Uint8Array(await window.localNotes.readPdf(docAtExportStart.id));
        sourcePdfExportDoc = await PDFDocument.load(originalPdfBytes);
      } catch (error) {
        console.warn('Could not load the original PDF for vector page export. Falling back to rendered pages.', error);
      }
    }
    ...
    for (const page of docAtExportStart.pages) {
      ...
      let size: { width: number; baseHeight: number; totalHeight: number } | null = null;
      if (sourcePdfExportDoc && canCopyOriginalPdfPageForExport(page)) {
        try {
          const [copiedPage] = await exportDoc.copyPages(sourcePdfExportDoc, [Math.max(0, page.sourcePage - 1)]);
          if (copiedPage) {
            pdfPage = exportDoc.addPage(copiedPage);
            size = { width: pdfPage.getWidth(), baseHeight: pdfPage.getHeight(), totalHeight: pdfPage.getHeight() };
          }
        } catch (error) {
          console.warn('Could not copy PDF page into export. Falling back to rendered page.', error);
        }
      }

      if (!pdfPage || !size) {
        const rendered = await renderBackgroundWithoutImages(page, pdfSource, exportTheme, 2);
        const backgroundBytes = dataUrlToUint8Array(rendered.canvas.toDataURL('image/png'));
        const background = await exportDoc.embedPng(backgroundBytes);
        size = rendered.size;
        pdfPage = exportDoc.addPage([size.width, size.totalHeight]);
        pdfPage.drawImage(background, { x: 0, y: 0, width: size.width, height: size.totalHeight });
      }

      const overlayEntries: RenderableElement[] = [];
      const flushOverlayEntries = async () => {
        if (!overlayEntries.length || !pdfPage || !size) return;
        const overlayCanvas = await renderElementOverlayGroup(overlayEntries.splice(0), size, 2);
      ...
      for (const entry of orderedElements(annotations, true)) {
        if (entry.type === 'image') {
          await flushOverlayEntries();
          try {
            const imageBytes = dataUrlToUint8Array(entry.item.dataUrl);
            const image = /^data:image\/jpe?g/i.test(entry.item.dataUrl)
              ? await exportDoc.embedJpg(imageBytes)
              : await exportDoc.embedPng(imageBytes);
    ...
    const sidecar = await buildEditableSidecar(docAtExportStart);
  ...
  async function handleExportPdf() {
    ...
    try {
      ...
      if (bytes) {
        ...
        const detail = error instanceof Error && error.message ? error.message : 'Unknown export error';
        const message = `PDF export failed: ${detail}. Reopen the document and try Save PDF copy again.`;
        setPdfSaveFailureMessage(message);
        setStatus(message);
      }
    } finally {
      ...
      <div className="continuous-page-shell" key={page.key}>
        ...
        >
          <canvas
            ...
            style={{ width: view.width * zoom, height: view.totalHeight * zoom }}
          />
          ...
          <canvas
            ...
            style={{ ...inkCanvasStyle, width: view.width * zoom, height: view.totalHeight * zoom }}
  ...
  const saveFailureVisible = Boolean(pdfSaveFailureMessage);
...
Answer: **x = ${formatNumber(r1)}** or **x = ${formatNumber(r2)}**.`;
  ...
  async function sendAiChatMessage() {
    ...
    let tabId = activeAiChatTab?.id || '';
    if (!tabId) {
      tabId = ensureAiChatTabForDocument(activeDocRef.current);
    }
    if (!tabId) return;

    const userMessage: ChatMessage = { id: makeId(), role: 'user', text, createdAt: new Date().toISOString() };
    const nextHistory = [...aiChatMessages, userMessage].slice(-AI_CHAT_MEMORY_LIMIT);
    updateAiChatTabMessages(tabId, () => nextHistory);
    setAiChatInput('');

    const turnContextRoute = getAiChatTurnContextRoute(text, nextHistory);
    if (turnContextRoute.replyLocally) {
      const answer = buildDuxAiResponse(text, nextHistory);
    ...
    const pageContext = turnContextRoute.useCurrentPage ? await buildCurrentPageAiContext(text) : {};
    const aiQuestionMemory = buildAiQuestionMemoryContext(text, nextHistory);
    const context = {
      ...
      userIntent: text,
      aiTurnContextMode: turnContextRoute.mode,
      aiTurnContextNote: turnContextRoute.note,
      aiPriorQuestionMemory: aiQuestionMemory,
      ...pageContext
    };

    try {
      const response = await requestDuxAiAnswer(nextHistory, context);
      const questionMemory = normaliseAiQuestionMemory(response.questionMemory || buildAiPageQuestionMemory(pageContext));
      const questionNumbers = extractAiQuestionNumbers(`${text}\n${response.answer}\n${questionMemory}`);
      updateAiChatTabMessages(tabId, (messages) => [...messages.slice(-AI_CHAT_MEMORY_LIMIT + 1), {
        id: makeId(),
        role: 'assistant',
        text: response.answer,
        createdAt: new Date().toISOString(),
        questionMemory: questionMemory || undefined,
        questionNumbers
      }]);
    } catch (error) {
...
Offline note: I could not reach the Groq backend, so I used the built-in Dux AI helper for this reply.`;
  ...
  function renderFlashcardSide(card: Flashcard | undefined, side: 'front' | 'back') {
    if (!card) return <strong>No card loaded.</strong>;
    const text = side === 'front' ? card.front : (card.back || 'No answer added yet.');
    const media = side === 'front' ? normaliseFlashcardMedia(card.frontMedia) : normaliseFlashcardMedia(card.backMedia);
    return (
      <div className="flashcard-side-content">
        {text.trim() && <strong>{text}</strong>}
        {media.length > 0 && (
          <div className="flashcard-media-grid" aria-label={`${side} images`}>
            {media.map((item, index) => (
              <img key={`${item.name}-${index}`} src={item.dataUrl} alt={item.name || `Imported Anki image ${index + 1}`} />
            ))}
          </div>
        )}
        {!text.trim() && media.length === 0 && <strong>{side === 'front' ? 'No prompt added yet.' : 'No answer added yet.'}</strong>}
      </div>
    );
  }

  return (
    <div ref={appShellRef} className={`app-shell button-size-${buttonSizeMode} ${uiMode === 'dark' ? 'dark-ui' : ''} ${sidebarCollapsed ? 'sidebar-collapsed' : ''} ${saveFailureVisible ? 'save-failed' : ''} ${onboardingVisible && activeOnboardingStep?.animationClass ? activeOnboardingStep.animationClass : ''}`} style={appStyle}>
      {saveFailureVisible && (
        <div className="save-failure-overlay" role="alert" aria-live="assertive">
          <div className="save-failure-card">
            <strong>Save failed</strong>
            <span>{pdfSaveFailureMessage} Export a fresh PDF copy from here so the app chrome stays out of the way.</span>
            <button
              type="button"
      ...
      <main className="workspace">
        ...
        {!isQuickNoteDoc(activeDoc) && <section
          className="toolbar"
          ...
          onPointerOver={(event) => {
            ...
            setToolbarTooltip({
              text: target.dataset.tooltip || target.getAttribute('aria-label') || target.title,
          ...
          onMouseOver={(event) => {
            ...
            setToolbarTooltip((current) => {
              const text = target.dataset.tooltip || target.getAttribute('aria-label') || target.title;
              const top = rect.top + rect.height / 2;
              const left = rect.left - 10;
              if (current?.text === text && Math.abs(current.top - top) < 0.5 && Math.abs(current.left - left) < 0.5) return current;
          ...
          onBlurCapture={() => {
            window.requestAnimationFrame(() => {
              if (!(document.activeElement instanceof Element) || !document.activeElement.closest('.toolbar [data-tooltip]')) {
        ...
        >
          ...
          <div className="toolbar-quick-actions" aria-label="Scratch and Dux AI" data-tour-id="right-utility">
            ...
            <button className="ask-ai-toggle" data-tour-id="dux-ai" data-tooltip="Dux AI" onClick={toggleAiChatForCurrentDocument} title="Ask Dux AI" aria-label="Ask Dux AI" aria-expanded={aiChatOpen}><UtilityIcon name="ai" /></button>
          </div>

          {renderMathsPanel()}

          {createPortal(<div style={appStyle} className={`context-panel ${secondarySettingsVisible ? 'is-open' : 'is-collapsed'} ${selectedObjectSettingsVisible ? 'selected-object-settings' : ''} ${tool === 'lasso' ? 'lasso-settings-panel' : ''} ${isInking && (tool === 'pen' || tool === 'highlighter' || tool === 'eraser' || tool === 'shape') ? 'is-inking-fade' : ''}`} aria-hidden={!secondarySettingsVisible}>
        ...
        {toolbarTooltip && (
          <div className="toolbar-floating-tooltip" style={{ top: toolbarTooltip.top, left: toolbarTooltip.left }}>
            {toolbarTooltip.text}
          </div>
        )}
        ...
        {activeDoc && !isQuickNoteDoc(activeDoc) && zoom <= 1.35 && (
        ...
        <section
          ...
          onPointerMove={rememberStagePointer}
          onContextMenu={(event) => {
        ...
        <section className="bottom-bar" aria-label="Workspace status and controls" data-tour-id="bottom-bar">
              ...
              {tipPopupOpen && (
                <div ref={tipPopupRef} className="tip-popup" style={tipPopupPosition ? ({ '--tip-left': `${tipPopupPosition.left}px`, '--tip-bottom': `${tipPopupPosition.bottom}px` } as CSSProperties) : undefined} role="dialog" aria-label="Study tip">
                  ...
                  <div className="tip-popup-actions">
                    ...
                    <button type="button" onClick={openAiChatForCurrentDocument}>Ask Dux AI</button>
          ...
          <div className="bottom-control-group" aria-label="Page, zoom and AI controls">
            ...
            <label className="zoom-control" title="Zoom canvas">
              Zoom
              <input type="range" min={MIN_PAGE_ZOOM} max={MAX_PAGE_ZOOM} step="0.05" value={zoom} onChange={(event) => scheduleZoomChange(Number(event.target.value), getLastStagePointerZoomAnchor())} />
      ...
      {aiChatOpen && (
        <div
          ref={aiChatPanelRef}
          className={`ai-chat-panel ai-chat-panel-floating${aiChatFrameActive ? ' dragging' : ''}`}
          style={{
            left: aiChatFrame.left,
            top: aiChatFrame.top,
            width: aiChatFrame.width,
            height: aiChatFrame.height,
            '--ai-chat-font-scale': aiChatZoom
          } as CSSProperties}
          onPointerDown={(event) => handleAiChatFramePointerDown(event, 'move')}
          onPointerMove={handleAiChatFramePointerMove}
          onPointerUp={handleAiChatFramePointerEnd}
          onPointerCancel={handleAiChatFramePointerEnd}
        >
          <div className="ai-chat-header">
            <div>
              <strong>Dux AI</strong>
              <span>{activeAiChatTab?.documentName || 'HSC study assistant'}</span>
            </div>
            <div className="ai-chat-header-actions">
              <div className="ai-chat-zoom-controls" aria-label="Dux AI answer zoom">
                <button
                  type="button"
                  onClick={() => setAiChatZoomIndex((index) => clamp(index - 1, 0, AI_CHAT_ZOOM_LEVELS.length - 1))}
                  disabled={aiChatZoomIndex <= 0}
                  title="Make Dux AI text smaller"
                >
                  −
                </button>
                <span>{Math.round(aiChatZoom * 100)}%</span>
                <button
                  type="button"
                  onClick={() => setAiChatZoomIndex((index) => clamp(index + 1, 0, AI_CHAT_ZOOM_LEVELS.length - 1))}
                  disabled={aiChatZoomIndex >= AI_CHAT_ZOOM_LEVELS.length - 1}
                  title="Make Dux AI text bigger"
                >
                  +
                </button>
              </div>
              <button onClick={() => activeAiChatTab && updateAiChatTabMessages(activeAiChatTab.id, () => [])}>Clear chat</button>
              <button onClick={() => setAiChatOpen(false)}>×</button>
            </div>
          </div>
          <div className="ai-chat-tab-strip" role="tablist" aria-label="Dux AI chat tabs">
            {aiChatTabs.map((tab) => (
              <div key={tab.id} className={`ai-chat-tab${tab.id === activeAiChatTab?.id ? ' active' : ''}`}>
                <button
                  type="button"
                  className="ai-chat-tab-main"
                  role="tab"
                  aria-selected={tab.id === activeAiChatTab?.id}
                  title={`${tab.title} • ${tab.documentName}`}
                  onClick={() => setActiveAiChatTabId(tab.id)}
                >
                  <span>{tab.title}</span>
                  <small>{tab.messages.length ? `${tab.messages.length} msg` : 'new'}</small>
                </button>
                <button
                  type="button"
                  className="ai-chat-tab-delete"
                  aria-label={`Delete ${tab.title} chat`}
                  title="Delete chat"
                  onClick={(event) => {
                    event.stopPropagation();
                    deleteAiChatTab(tab.id);
                  }}
                >
                  ×
                </button>
            ...
            <button type="button" className="ai-chat-new-tab" onClick={createNewAiChatTabForCurrentDocument}>+ Chat</button>
          </div>
          <div ref={aiChatHistoryRef} className="ai-chat-history">
            {aiChatMessages.length === 0 && (
              <div className="ai-chat-welcome">Hi! This chat is linked to {activeAiChatTab?.documentName || 'your current document'}. Ask for the visible question, marking, HSC working, study help, or app help.</div>
            )}
            {aiChatMessages.map((message) => (
              <div key={message.id} className={`ai-chat-row ${message.role}`}>
                <div className={`ai-chat-bubble${message.role === 'assistant' && collapsedAiMessageIds.includes(message.id) ? ' is-collapsed' : ''}`}>
                  {message.role === 'assistant' ? (
                    <>
                      ...
                      <div className="ai-answer-footer">
                        <div className="ai-answer-actions" aria-label="Dux AI answer actions">
                          <button type="button" onClick={() => void copyAiAnswer(message)}>Copy answer</button>
                          <button type="button" onClick={() => exportAiAnswer(message)}>Export answer</button>
                          <button type="button" onClick={() => toggleAiAnswerCollapsed(message.id)}>
                            {collapsedAiMessageIds.includes(message.id) ? 'Maximise' : 'Minimise'}
                          </button>
                        </div>
                        {message.createdAt && <time className="ai-chat-timestamp" dateTime={message.createdAt}>{formatChatMessageTime(message.createdAt)}</time>}
                      </div>
                    </>
                  ) : (
                    <>
          ...
          {(['nw', 'ne', 'sw', 'se'] as const).map((corner) => (
            <button
              key={corner}
              type="button"
              className={`ai-chat-resize-handle ai-chat-resize-${corner}`}
              aria-label={`Resize Dux AI window from ${corner.toUpperCase()} corner`}
              title="Resize Dux AI"
              onPointerDown={(event) => handleAiChatFramePointerDown(event, `resize-${corner}` as AiChatFrameDragMode)}
            />
          ))}
        </div>
      )}
      ...
      <input
        ref={aiScheduleFileInputRef}
        type="file"
        className="hidden-file-input"
        accept=".pdf,.png,.jpg,.jpeg,.webp,.txt,.csv,.tsv,.ics,.json,.md,.markdown,image/png,image/jpeg,image/webp,text/*,application/pdf,text/calendar"
        onChange={handleAiScheduleFile}
      />
        ...
        <div className="modal-backdrop" role="dialog" aria-modal="true" onClick={() => { setFlashcardModalOpen(false); setFlashcardGeneratorOpen(false); setFlashcardCreateOpen(false); resetFlashcardReview(); }}>
          <div className="flashcard-review-modal" onClick={(event) => event.stopPropagation()}>
            ...
            <input
              ...
              type="file"
              className="hidden-file-input"
              accept=".apkg,.apkg2,.apkj,.txt,.csv,.tsv,application/octet-stream,application/zip,text/*"
            ...
            {!reviewStarted ? (
                      ...
                      {ankiImportPreview && (
                        <>
                          ...
                          <div className="anki-import-preview">
                            ...
                            <span>{flashcardSideTextPreview(ankiImportPreview.cards[0]?.front || '', ankiImportPreview.cards[0]?.frontMedia)} → {flashcardSideTextPreview(ankiImportPreview.cards[0]?.back || '', ankiImportPreview.cards[0]?.backMedia)}</span>
                          </div>
                          <div className="modal-actions generator-actions">
                  ...
                  <div className="card-list-mini">
                    {flashcards.length === 0 ? <p className="empty-message">No flashcards yet.</p> : flashcards.slice(0, 80).map((card) => (
                      <article key={card.id} className="mini-card-row">
                        <div>
                          <b>{flashcardSideTextPreview(card.front, card.frontMedia)}</b>
              ...
              <div className="flashcard-review-panel">
                ...
                <button className={`review-card ${reviewFlipped ? 'flipped' : ''}`} onClick={() => setReviewFlipped((value) => !value)}>
                  ...
                  {renderFlashcardSide(reviewQueue[reviewIndex], reviewFlipped ? 'back' : 'front')}
                </button>
                {reviewFlipped && (
        ...
        <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Dux Notes help" onClick={() => setHelpOpen(false)}>
          <div className="create-modal help-modal" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="help-grid">
              <section>
                ...
                <ul>
                  ...
                  <li><b>Pen</b> writes freehand with low-latency live ink. Pick colour, width and opacity in the settings panel.</li>
                  <li><b>Scribble erase</b> lets you scratch out handwriting: draw at least four quick zigzags across an existing stroke, hold the pen still for a third of a second at the end, then lift.</li>
                  <li><b>Highlighter</b> marks notes with a wider transparent stroke and keeps the live stroke continuous while you write.</li>
                  ...
                  <li><b>Delete selected images</b> with Backspace or Delete when the Select tool has the image active.</li>
                  ...
                  <li><b>Maths</b> opens symbols, templates, fraction/power builders and a Cartesian plane tool. Inserted planes can be selected, moved, resized and exported.</li>
              ...
              <section>
                ...
                <ul>
                  ...
                  <li><b>Zoom</b> changes the page size up to 300%. On iPad, pinch with two fingers. On laptop trackpads, pinch over the notebook to zoom the page instead of the browser.</li>
                  ...
                  <li><b>Bookmarks rail</b> appears beside the page thumbnails at normal zoom and hides at higher zoom so it does not cover the question.</li>
              ...
              <section>
                ...
                <ul>
                  <li>Use Apple Pencil or mouse to write. The app uses coalesced pointer points for smoother strokes.</li>
              ...
              <section>
                ...
                <ul>
                  <li><b>Dux AI</b> answers study questions, explains topics, checks working and marks responses using an HSC-style criteria and mark breakdown.</li>
                  <li><b>Floating Dux AI</b> can be dragged by its header and resized from the bottom-right corner. Your sent messages show timestamps.</li>
                  <li><b>PDF and handwriting context</b> lets Dux AI read the current page image, printed question text and visible handwritten working when the backend supports vision.</li>
                  <li><b>Flashcards</b> lets you add cards, generate local cards, import Anki decks with supported media and review with spaced-repetition ratings.</li>
                  <li><b>Study schedule</b> builds study blocks. If you give time slots or upload a timetable file/image, it places the work inside those slots.</li>
              ...
              <section>
                <h3>AI marking</h3>
                <ul>
                  <li>Ask “mark this out of 3” or “mark my working for question 12”. Dux AI will identify the question, read your visible answer and return a mark breakdown.</li>
                  <li>The marking format is: visible student answer, correct answer, mark out of the total, criteria table, lost marks and an HSC-standard fix.</li>
                  <li>For maths and science, it separates method, substitution, calculation, units and final-answer marks.</li>
                  <li>For English, SOR and extended responses, it checks argument, evidence, analysis, terminology, judgement and relevance.</li>
                  <li>If handwriting or the official marking guideline is not visible, it marks conservatively and tells you what it could not read.</li>
                </ul>
              </section>
              ...
              <section>
                ...
                <ul>
                  ...
                  <li><b>Accessibility</b> changes button size for compact, standard or larger touch targets.</li>
                  <li><b>Tags</b> organise documents. You can create and delete custom tags from the tag menu.</li>
                </ul>
              </section>
            ...
            <div className="help-tips">
              <strong>Fast workflow:</strong> Import or create a PDF, pick Pen, write, make four zigzags and pause briefly to scratch-erase mistakes, use two fingers to move around, bookmark key pages in Contents, ask Dux AI to mark visible working, customise the theme in Settings, then Save PDF when finished.
            </div>
            ...
            {aiPanelOpen && (
              <section className="ai-generator-panel">
                ...
                <div
                  ...
                  onDrop={handleAiScheduleFileDrop}
                >
                  <div>
                    <strong>Attach a timetable or study file</strong>
                    <span>{aiScheduleFileName ? `${aiScheduleFileName}${aiScheduleFileText || aiScheduleFileImageDataUrl ? ' attached' : ' has no readable text yet'}` : 'Drop a screenshot here, or upload PDF, PNG, JPEG, CSV, TXT, ICS, JSON or Markdown.'}</span>
                  </div>
                  <div className="ai-upload-actions">
                    <button className="secondary-button" type="button" onClick={() => aiScheduleFileInputRef.current?.click()} disabled={aiLoading}>
                      Upload file
                    </button>
                    {aiScheduleFileName && <button className="ghost-button" type="button" onClick={clearAiScheduleFile}>Remove</button>}
                  </div>
                </div>
                ...
                <div className="field-grid ai-date-grid">
                  <label className="field-block">
                    Schedule start date
                    <input type="date" value={aiStartDate} onChange={(event) => handleAiStartDateChange(event.target.value)} />
                  </label>
                  <label className="field-block">
                    Schedule end date
                    <input type="date" value={aiEndDate} min={aiStartDate} onChange={(event) => handleAiEndDateChange(event.target.value)} />
                  </label>
                </div>
              ...
              <section className="study-editor-panel">
                ...
                <div className="study-form-card">
                  ...
                  <div className="field-grid compact">
                    <label className="field-block">
                      Start
                      <input type="time" value={studyDraft.startTime} onChange={(event) => handleStudyDraftStartTimeChange(event.target.value)} />
                    </label>
                    <label className="field-block">
                      End
                      <input type="time" value={studyDraft.endTime} min={minimumEndTimeForStart(studyDraft.startTime)} onChange={(event) => handleStudyDraftEndTimeChange(event.target.value)} />
                    </label>
                  </div>
