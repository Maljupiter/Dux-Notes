...
function safeReadLegacyLibrary(): Library {
  try {
...
function tryWriteLegacyLibrary(library: Library) {
  try {
    window.localStorage.setItem(BROWSER_LIBRARY_KEY, JSON.stringify({ documents: library.documents || [] }));
  } catch (error) {
    // IndexedDB is the primary browser store; localStorage is only kept as a small legacy fallback.
    console.warn('Browser legacy localStorage mirror could not be written.', error);
  }
}
...
async function withPdfStore<T>(mode: IDBTransactionMode, action: (store: IDBObjectStore) => IDBRequest<T>): Promise<T> {
  return withBrowserStore(PDF_STORE, mode, action);
}

async function withBrowserStore<T>(storeName: string, mode: IDBTransactionMode, action: (store: IDBObjectStore) => IDBRequest<T>): Promise<T> {
...
async function readLibrary(): Promise<Library> {
  if (!window.indexedDB) return safeReadLegacyLibrary();
  try {
    const stored = await withBrowserStore<Library | undefined>(LIBRARY_STORE, 'readonly', (store) => store.get(LIBRARY_RECORD_KEY));
    if (stored && Array.isArray(stored.documents)) return { documents: stored.documents };

    const legacy = safeReadLegacyLibrary();
    if (legacy.documents.length) {
      try {
        await writeLibrary(legacy);
      } catch (error) {
        console.warn('Could not migrate browser library into IndexedDB.', error);
      }
    }
    return legacy;
  } catch (error) {
    console.warn('Browser IndexedDB library could not be read. Falling back to legacy localStorage.', error);
    return safeReadLegacyLibrary();
  }
}

async function writeLibrary(library: Library) {
  const next = { documents: library.documents || [] };
  if (!window.indexedDB) {
    tryWriteLegacyLibrary(next);
    return;
  }
  await withBrowserStore(LIBRARY_STORE, 'readwrite', (store) => store.put(next, LIBRARY_RECORD_KEY));
}
...
export function ensureBrowserLocalNotes() {
  ...
  window.localNotes = {
    ...
    async savePdfExport(defaultFileName: string, pdfBytes: Uint8Array | ArrayBuffer | number[], editableSidecar?: unknown): Promise<string | null> {
      const fileName = defaultFileName?.toLowerCase().endsWith('.pdf') ? defaultFileName : `${defaultFileName || 'Dux Notes Export'}.pdf`;
      const bytes = Array.isArray(pdfBytes) ? new Uint8Array(pdfBytes) : pdfBytes;
      downloadBrowserFile(fileName, bytes, 'application/pdf');
    ...
    async savePdfExportToFolder(_folderPath: string, defaultFileName: string, pdfBytes: Uint8Array | ArrayBuffer | number[], editableSidecar?: unknown): Promise<string | null> {
